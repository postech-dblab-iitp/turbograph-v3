...
for query_num in $queries; do
    ...
    warmup_output=$(timeout 3600s ../../build-release/s62-client/TurboGraph-S62 --workspace:${database_path} --query:"$query_str" --disable-merge-join --join-order-optimizer:greedy)
    output_str=$(timeout 3600s ../../build-release/s62-client/TurboGraph-S62 --workspace:${database_path} --query:"$query_str" --disable-merge-join --num-iterations:3 --join-order-optimizer:greedy)
    exit_status=$?
