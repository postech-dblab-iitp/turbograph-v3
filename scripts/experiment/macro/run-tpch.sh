...
for query_num in $queries; do
    ...
    warmup_output=$(timeout 3600s ../../build-release/s62-client/S62-CLI --workspace:${database_path} --query:"$query_str" --disable-merge-join --join-order-optimizer:exhaustive)
    output_str=$(timeout 3600s ../../build-release/s62-client/S62-CLI --workspace:${database_path} --query:"$query_str" --disable-merge-join --num-iterations:3 --join-order-optimizer:exhaustive)
    exit_status=$?
