	...
	static s62::Value DeserializeOrcaByteArrayIntoDuckDBValue(uint32_t type_id, int32_t type_modifier, const void* mem_ptr, uint64_t length) {
		...
		switch (duckdb_type) {
			...
			case s62::LogicalTypeId::UINTEGER: {
				D_ASSERT(length == 4 || length == 0);
				if (length == 0) {
					return s62::Value(s62::LogicalType::UINTEGER);
				} else {
					uint32_t value = *((uint32_t*)mem_ptr);
					return s62::Value::UINTEGER((uint32_t)value);
				}
			}
			case s62::LogicalTypeId::UBIGINT: {
				D_ASSERT(length == 8 || length == 0);
				if (length == 0) {
					return s62::Value(s62::LogicalType::UBIGINT);
				} else {
					uint64_t value = *((uint64_t*)mem_ptr);
					return s62::Value::UBIGINT(value);
				}
			}
			case s62::LogicalTypeId::DOUBLE: {
				D_ASSERT(length == 8 || length == 0);
				if (length == 0) {
					return s62::Value(s62::LogicalType::DOUBLE);
				} else {
					double value = *((double*)mem_ptr);
					return s62::Value::DOUBLE(value);
				}
			}
			case s62::LogicalTypeId::HUGEINT: {
				D_ASSERT(length == 8 || length == 0);
				if (length == 0) {
					return s62::Value(s62::LogicalType::HUGEINT);
				} else {
					uint64_t value = *((int64_t*)mem_ptr);
					return s62::Value::HUGEINT(value);
				}
			}
			case s62::LogicalTypeId::VARCHAR: {
				if (length == 0) {
					return s62::Value(s62::LogicalType::VARCHAR);
				} else {
					string str_value((char*)mem_ptr);
					s62::string_t value(str_value);
					return s62::Value(value);
				}
			}
			case s62::LogicalTypeId::ID: {
				D_ASSERT(length == 8 || length == 0);
				if (length == 0) {
					return s62::Value(s62::LogicalType::ID);
				} else {
					uint64_t value = *((uint64_t*)mem_ptr);
					return s62::Value::ID(value);
				}
			}
			case s62::LogicalTypeId::BOOLEAN: {
				int8_t value = *((int8_t*)mem_ptr);
				return s62::Value::BOOLEAN(value);
			}
			case s62::LogicalTypeId::DATE: {
				int32_t value = *((int32_t *)mem_ptr);
			...
			case s62::LogicalTypeId::PATH: {
				return s62::Value::LIST(s62::LogicalTypeId::UBIGINT, {});
			}
			case s62::LogicalTypeId::LIST: {
				if (length == 0) {
					D_ASSERT(type_modifier >= std::numeric_limits<uint8_t>::min() && type_modifier <= std::numeric_limits<uint8_t>::max());
					s62::LogicalTypeId child_type_id = (s62::LogicalTypeId)type_modifier;
					return s62::Value::LIST(child_type_id, {});
				} else {
					D_ASSERT(type_modifier >= std::numeric_limits<uint8_t>::min() && type_modifier <= std::numeric_limits<uint8_t>::max());
					s62::LogicalTypeId child_type_id = (s62::LogicalTypeId)type_modifier;
					...
					case s62::LogicalTypeId::VARCHAR: {
						...
						for (auto i = 0; i < num_strings; i++) {
							...
							s62::string_t value(str_value);
							list_values.push_back(s62::Value(value));
							accm_bytes += str_size;
						}
						return s62::Value::LIST(s62::LogicalTypeId::VARCHAR, list_values);
					}
					default:
						D_ASSERT(false);
					}
