...
new_list(NodeTag type)
{
	...
	new_head = (ListCell *) calloc(sizeof(*new_head), 1);
	new_head->next = NULL;
	/* new_head->data is left undefined! */

	// new_list = (List *) palloc(sizeof(*new_list));
	new_list = (List *) calloc(sizeof(*new_list), 1);
	new_list->type = type;
	new_list->length = 1;
...
new_tail_cell(List *list)
{
	...
	new_tail = (ListCell *) calloc(sizeof(*new_tail), 1);
	new_tail->next = NULL;
...
add_new_cell(List *list, ListCell *prev_cell)
{
	...
	new_cell = (ListCell *) calloc(sizeof(*new_cell), 1);
...
list_member(const List *list, const void *datum)
{
	...
	{
		Assert(false);
		// if (equal(lfirst(cell), datum)) // TODO
		// 	return true;
	}
...
list_copy(const List *oldlist)
{
	...
	while (oldlist_cur)
	{
		...
		newlist_cur = (ListCell *) calloc(sizeof(*newlist_cur), 1);
