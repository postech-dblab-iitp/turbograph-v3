...
class Turbo_bin_aio_handler {
   public:
    ...
    Turbo_bin_aio_handler(const char *file_name,
    ...
    {
        if (my_core_id_ == -1) {
            my_core_id_ = std::atomic_fetch_add(
                (std::atomic<int64_t> *)&(core_counts_), 1L);
        }
    ...
    static void InitializeIoInterface()
    {
        ...
        if (per_thread_aio_interface_read.get(my_core_id_) == nullptr) {
            per_thread_aio_interface_read.get(my_core_id_) =
                DiskAioFactory::GetPtr()->CreateAioInterface(max_num_ongoing,
                                                             aio_tid);
        }
        if (per_thread_aio_interface_write.get(my_core_id_) == nullptr) {
            per_thread_aio_interface_write.get(my_core_id_) =
                DiskAioFactory::GetPtr()->CreateAioInterface(max_num_ongoing,
                                                             aio_tid);
        }
    }
    ...
    static diskaio::DiskAioInterface *&GetMyDiskIoInterface(bool read)
    {
        ...
        assert(my_core_id_ >= 0 &&
               my_core_id_ < MAX_NUM_PER_THREAD_DATASTRUCTURE);
        if (read) {
    ...
    {
        for (int core_id = 0; core_id < MAX_NUM_PER_THREAD_DATASTRUCTURE;
             core_id++) {
            if (read) {
                diskaio::DiskAioInterface *my_io =
                    per_thread_aio_interface_read.get(core_id);
                if (my_io != NULL) {
            ...
            if (write) {
                diskaio::DiskAioInterface *my_io =
                    per_thread_aio_interface_write.get(core_id);
                if (my_io != NULL) {
                    WaitMyPendingDiskIO(my_io, my_io->GetNumOngoing());
                }
            }
    ...
    static void WaitMyPendingDiskIO(diskaio::DiskAioInterface *my_io,
                                    int num_to_complete)
    {
        int backoff = 1;
        num_to_complete = (num_to_complete > my_io->GetNumOngoing())
                              ? my_io->GetNumOngoing()
                              : num_to_complete;
        do {
...
#pragma omp parallel num_threads(DiskAioParameters::NUM_THREADS)
                          ...
                          bool delete_if_exist = false, bool o_direct = false)
    {
        ...
        if (file_id < 0) {
            fprintf(stdout,
                    "[Turbo_bin_aio_handler::OpenFile] Fail to open file %s\n",
                    file_name);
    ...
    ReturnStatus Append(
        std::int64_t size_to_append, char *data, void *func,
        const std::function<void(diskaio::DiskAioInterface *)> &wait_cb = {})
    {
        InitializeIoInterface();
    ...
    ReturnStatus Read(int64_t offset_to_read, int64_t size_to_read, char *data,
                      ...
                      bool construct_next = false, bool mark_changed = false)
    {
        if (is_reserved)
            return ReturnStatus::OK;
    ...
    ReturnStatus Read(int64_t offset_to_read, int64_t size_to_read, char *data,
                      void *caller, void *func, void *buf_to_construct = NULL,
                      bool construct_next = false, bool mark_changed = false)
    {
        InitializeIoInterface();
        diskaio::DiskAioInterface *my_io = GetMyDiskIoInterface(true);
        Read(offset_to_read, size_to_read, data, caller, func, my_io,
             buf_to_construct, construct_next, mark_changed);
    }

    // TODO - remove buf_to_construct, construct_next, change API to get templated user-defined request
    void ReadWithSplittedIORequest(int64_t offset_to_read, int64_t size_to_read,
                                   char *data, void *caller, void *func,
                                   diskaio::DiskAioInterface *my_io)
    {
        if (is_reserved)
            return;
        ...
        while (size_to_read != 0) {
            cur_io_size = size_to_read > MAX_IO_SIZE_PER_RW ? MAX_IO_SIZE_PER_RW
                                                            : size_to_read;
                                   ...
                                   char *data, void *caller, void *func)
    {
        InitializeIoInterface();
        diskaio::DiskAioInterface *my_io = GetMyDiskIoInterface(true);
        ReadWithSplittedIORequest(offset_to_read, size_to_read, data, caller,
                                  func, my_io);
    }
    ...
    void FlushAllBlocking()
    {
        ...
        while (size_to_flush != 0) {
            cur_io_size = size_to_flush > MAX_IO_SIZE_PER_RW
                              ? MAX_IO_SIZE_PER_RW
                              : size_to_flush;
    ...
    char *CreateMmap(bool write_enabled)
    {
        ...
        assert(file_size_ > 0);
        file_mmap = (char *)mmap64(NULL, file_size_, mmap_prot, mmap_flag,
                                   file_descriptor, 0);
    ...
    int64_t GetRequestedSize() { return requested_size_; }
...
#pragma omp parallel num_threads(DiskAioParameters::NUM_THREADS)
    ...
    void SetCanDestroy(bool can_destroy) { delete_when_close = can_destroy; }

    uint8_t *GetDataPtr() { return aligned_data_ptr; }

    void SetDataPtr(uint8_t *data_ptr_) { aligned_data_ptr = data_ptr_; }
   ...
   private:
    ...
    static per_thread_lazy<diskaio::DiskAioInterface *>
        per_thread_aio_interface_read;
    static per_thread_lazy<diskaio::DiskAioInterface *>
        per_thread_aio_interface_write;
