...
struct DiskAioRequestUserInfo {
	...
	int32_t frame_id=-1; /* the id of buffer frame */
	int32_t task_id=-1;
	void* caller=0;
	void* func = NULL; /* a pointer to callback function */
	bool do_user_cb=false;
	bool do_user_only_req_cb=false;
	char* read_buf=0;
	void* read_my_io=NULL;
    void* buf_to_construct=NULL; // TODO need to removed later
    bool construct_next=false; // TODO need to removed later
    bool mark_changed=false; // TODO need to removed later
