...
/*
 * Design of the Turbo_bin_aio_handler
 *
 * This class is designed to obtain performance gains by overlapping CPU and 
 * I/O by performing I/O asynchronously by utilizing the function of disk aio.
 * The APIs of this class are based on the APIs of Turbo_bin_io_handler. Currently,
 * only Read() and Append() functions are provided.
 *
 * The main function is the Read() function. The function takes the offset in the 
 * binary file of the data to be read, the size to read, and the buffer to read 
 * as arguments. Also, it takes the callback function as an argument. After reading 
 * the file into the buffer through disk aio, the callback function is executed.
 *
 * TODO - Currently, this class is only used for the VersionedArray class. There 
 * are APIs designed to be used only for that class (Read). This could be solved 
 * by taking a templated user-defined request as input and allowing the request 
 * to contain the necessary information for arbitrary class.
 */
  ...
  public:
    ...
    static void InitializeCoreIds() {
      if (my_core_id_ == -1) {
        ...
        //if (UserArguments::tmp == 0) fprintf(stdout, "[%ld] [Turbo_bin_aio_handler::InitializeCoreIds] Alloc %ld\n", UserArguments::tmp, my_core_id_);
      }
      ALWAYS_ASSERT(my_core_id_ >= 0);
      ALWAYS_ASSERT(my_core_id_ < MAX_NUM_PER_THREAD_DATASTRUCTURE);
    }
    ...
    static diskaio::DiskAioInterface*& GetMyDiskIoInterface(bool read, int core_id_) {
      InitializeCoreIds();
      ALWAYS_ASSERT (core_id_ != -1);
      ALWAYS_ASSERT (core_id_ >= 0 && core_id_ < MAX_NUM_PER_THREAD_DATASTRUCTURE);
      if (read) {
...
#pragma omp parallel num_threads(UserArguments::NUM_THREADS)
    ...
    void Close(bool rm = false) {
      if (file_id >= 0) {
        ALWAYS_ASSERT(DiskAioFactory::GetPtr() != NULL);
        DiskAioFactory::GetPtr()->CloseAioFile(file_id);
        file_id = -1;
      }
    ...
    ReturnStatus OpenFile(const char* file_name, bool create_if_not_exist = false, bool write_enabled = false, bool delete_if_exist = false, bool o_direct = false) {
      ...
      if (o_direct) flag = flag | O_DIRECT; //XXX for tmpfs
    ...
    ReturnStatus OpenFileTemp(const char* file_name, bool create_if_not_exist = false, bool write_enabled = false, bool delete_if_exist = false, bool o_direct = false) {
      ...
      INVARIANT(f != -1);
      file_path = std::string(file_name);
      //INVARIANT(file_descriptor == file_descriptor_temp);
      return OK;
    }

    ReturnStatus Append(std::int64_t size_to_append, char* data, void* func, const std::function<void(diskaio::DiskAioInterface*)>& wait_cb = {}) {
      //if (UserArguments::tmp == 0) fprintf(stdout, "[%ld] [Turbo_bin_aio_handler::Append] %s / my_core_id_ = %ld\n", UserArguments::tmp, file_path.c_str(), my_core_id_);
      InitializeIoInterface();
      ALWAYS_ASSERT(size_to_append % 512 == 0);
      ALWAYS_ASSERT(file_size() % 512 == 0);
      ALWAYS_ASSERT(((uintptr_t)data) % 512 == 0);
    ...
    ReturnStatus Read(int64_t offset_to_read, int64_t size_to_read, char* data, void* caller, void* func, diskaio::DiskAioInterface* my_io, void* buf_to_construct=NULL, bool construct_next=false, bool mark_changed=false) {
      //if (UserArguments::tmp == 0) fprintf(stdout, "[%ld] [Turbo_bin_aio_handler::Read] %s / my_core_id_ = %ld\n", UserArguments::tmp, file_path.c_str(), my_core_id_);
      ALWAYS_ASSERT (size_to_read > 0);
      ALWAYS_ASSERT (size_to_read % 512 == 0);
      ALWAYS_ASSERT (offset_to_read % 512 == 0);
      ALWAYS_ASSERT (((uintptr_t)data) % 512 == 0);
      AioRequest req;
      req.buf = data;
      ...
      req.user_info.func = func;
      req.user_info.buf_to_construct = buf_to_construct; // XXX
      req.user_info.construct_next = construct_next; // XXX
      req.user_info.mark_changed = mark_changed; // XXX
    ...
    ReturnStatus Read(int64_t offset_to_read, int64_t size_to_read, char* data, void* caller, void* func, void* buf_to_construct=NULL, bool construct_next=false, bool mark_changed=false) {
      InitializeIoInterface();
      diskaio::DiskAioInterface* my_io = GetMyDiskIoInterface(true);
      return Read(offset_to_read, size_to_read, data, caller, func, my_io, buf_to_construct, construct_next, mark_changed);
    }
    ...
    void DestructMmap() {
      ...
      INVARIANT (file_size_ >= 0);
      PCHECK(munmap(file_mmap, file_size_) == 0);
      file_mmap = NULL;
    }
