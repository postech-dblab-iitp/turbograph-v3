...
Retry:
			...
			while(write_size < 8) {
				...
				if (result < 0) {
                    ...
                    if (retry) {
                        ...
                        reaccept_socket(partition_id);
                        fprintf(stdout, "[turbo tcp]Send to client %lld --> %lld Return -1\n", PartitionStatistics::my_machine_id(), partition_id);
                        return -1;
                    }
					//goto Retry;
				}
			...
			while(write_size < size_to_send) {
				...
				ALWAYS_ASSERT(result <= size_to_send - write_size);
				if (result < 0) {
                    //if (result == EAGAIN || result == EWOULDBLOCK) {
                    if (retry) {
                        fprintf(stdout, "[turbo tcp] Send to client error, from %lld to %lld, size_to_send = %lld, sent_size = %lld\n", PartitionStatistics::my_machine_id(), partition_id, size_to_send, write_size);
			...
			if(write_size != size_to_send)
				fprintf(stdout, "[turbo tcp] From %lld to %lld, Write size = %lld, Size to send = %lld\n", PartitionStatistics::my_machine_id(), partition_id, write_size, size_to_send);
			INVARIANT(write_size == size_to_send);
			return write_size;
		}
...
Retry:
			...
			while(read_size < 8) {
				...
				if (result < 0) {
                    ...
                    if (retry) {
                        ...
                        fprintf(stdout, "[turbo tcp] Recv from server error, from %lld to %lld, size_to_recv = %lld, read_size = %lld\n", partition_id, PartitionStatistics::my_machine_id(), size_to_recv, read_size);
                        perror("[turbo tcp] recv from server size_to_recv error");
                        reconnect_socket(partition_id);
                        fprintf(stdout, "[turbo tcp] Recv from server %lld <-- %lld ReStart\n", PartitionStatistics::my_machine_id(), partition_id);
                        goto Retry;
                    } else {
                        fprintf(stdout, "[turbo tcp] Recv from server error, from %lld to %lld, size_to_recv = %lld, read_size = %lld\n", partition_id, PartitionStatistics::my_machine_id(), size_to_recv, read_size);
                        perror("[turbo tcp] recv from server size_to_recv error");
                        ...
                        reconnect_socket(partition_id);
                        fprintf(stdout, "[turbo tcp] Recv from server %lld <-- %lld Return -1\n", PartitionStatistics::my_machine_id(), partition_id);
                        return -1;
                    }
				}
				read_size += result;
			...
			while(read_size < size_to_recv) {
				...
				ALWAYS_ASSERT(result <= size_to_recv - read_size);
				if (result < 0) {
                    //if (result == EAGAIN || result == EWOULDBLOCK) {
                    if (retry) {
                        //fprintf(stdout, "[%ld<-%ld][turbo tcp] recv_from_server EAGAIN || EWOULDBLOCK\n", PartitionStatistics::my_machine_id(), partition_id);
			...
			if(read_size != size_to_recv)
				fprintf(stdout, "[turbo tcp] From %lld to %lld, Read size = %lld, Size to recv = %lld\n", partition_id, PartitionStatistics::my_machine_id(), read_size, size_to_recv);
			INVARIANT(read_size == size_to_recv);
...
Retry:
			...
			while(read_size < 8) {
				...
				if (result < 0) {
                    //if (result == EAGAIN || result == EWOULDBLOCK) {
                    if (retry) {
                        fprintf(stdout, "[turbo tcp] Recv from client error with errno %d, from %lld to %lld, size_to_recv = %lld, read_size = %lld\n", errno, partition_id, PartitionStatistics::my_machine_id(), size_to_recv, read_size);
            ...
            if (size_to_recv > max_bytes || size_to_recv < 0) {
                fprintf(stdout, "[turbo tcp] Recv from client error with errno %d, from %lld to %lld, size_to_recv = %lld, read_size = %lld, max_bytes = %ld\n", errno, partition_id, PartitionStatistics::my_machine_id(), size_to_recv, read_size, max_bytes);
                return -1;
            }
...
#pragma omp parallel num_threads(2)
	...
	private:
		ReturnStatus reconnect_socket(int partition_id) {
			...
			ling.l_onoff = 1;
			ling.l_linger = 0;
			fprintf(stdout, "[turbo_tcp] Machine %lld try to reconnect socket with Machine %lld\n", PartitionStatistics::my_machine_id(), partition_id);
		...
		ReturnStatus reaccept_socket(int partition_id) {
			...
			ling.l_linger = 0;
			//XXX check really disconnected
			fprintf(stdout, "[turbo_tcp] Machine %lld try to reaccept socket Machine %lld\n", PartitionStatistics::my_machine_id(), partition_id);
			close(client_socket_for_server[partition_id]);
			sleep(1);
			while(listen(server_socket[partition_id], 24) == -1) {}
			fprintf(stdout, "[turbo_tcp] Machine %lld listen success Machine %lld\n", PartitionStatistics::my_machine_id(), partition_id);
        ...
        int64_t error_handling_mpi_send(const char* buf, int64_t size_to_send, int partition_id) {
            ...
            INVARIANT(mpi_error == MPI_SUCCESS);
        ...
        int64_t error_handling_mpi_recv(const char* buf, int partition_id) {
            ...
            INVARIANT(mpi_error == MPI_SUCCESS);
            MPI_Get_count(&stat, MPI_CHAR, &recv_bytes);
            INVARIANT(recv_bytes == size_to_recv);
            return size_to_recv;
        }	
