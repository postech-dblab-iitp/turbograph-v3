...
class per_partition_elem {
   public:
    ...
    ~per_partition_elem() { ALWAYS_ASSERT(win_ == MPI_WIN_NULL); }
    ...
    {
        if (win_ == MPI_WIN_NULL) {
            MPI_Win_create(elems_, sizeof(T) * MAX_NUM_MACHINES, sizeof(T),
                           MPI_INFO_NULL, MPI_COMM_WORLD, &win_);
        }
        wait_on();

        for (int i = 0; i < num_partitions__; i++) {
            if (i != my_partition_id__) {
                MPI_Put((char *)&elems_[my_partition_id__], sizeof(T), MPI_CHAR,
                        i, my_partition_id__, sizeof(T), MPI_CHAR, win_);
            }
        }
    ...
    void pull()
    {
        ...
        for (int i = 0; i < num_partitions__; i++) {
            if (i != my_partition_id__) {
                MPI_Get((char *)&elems_[i], sizeof(T), MPI_CHAR, i, i,
                        sizeof(T), MPI_CHAR, win_);
            }
        }
        wait_on();
    }

    inline T &operator[](std::size_t idx) { return elems_[idx]; }
    inline const T &operator[](std::size_t idx) const { return elems_[idx]; }

    inline T &mine() { return elems_[my_partition_id__]; }
    inline const T &mine() const { return elems_[my_partition_id__]; }
    void wait_on()
    {
        if (win_ != MPI_WIN_NULL) {
...
class per_partition_array {
   public:
    ...
    void make_array()
    {
        ...
        MPI_Win_create(pData_, sizeof(T) * total_size_, sizeof(T),
                       MPI_INFO_NULL, MPI_COMM_WORLD, &win_);
    }
    ...
    void push_data()
    {
        ...
        for (T_size_t i = 0; i < num_partitions__; i++) {
            if (i != my_partition_id__) {
                MPI_Put((char *)&pData_[disp],
                        sizes[my_partition_id__] * sizeof(T), MPI_CHAR, i, disp,
                        sizes_[my_partition_id__] * sizeof(T), MPI_CHAR, win_);
            }
        }
        wait_on();
    }
        ...
        for (i = 0; i < num_partitions__; i++) {
            if (i != my_partition_id__) {
                for (j = disp; j < disp + sizes_[i]; j++) {
                    MPI_Get((char *)&pData_[disp], sizes_[i] * sizeof(T),
                            MPI_CHAR, i, disp, sizes_[i] * sizeof(T), MPI_CHAR,
                            win_);
                }
            }
            disp += sizes_[i];
    ...
    void wait_on() { MPI_Win_fence(0, win_); }

    T_size_t &my_size() { return sizes_[my_partition_id__]; }

    per_partition_elem<T_size_t> &sizes() { return sizes_; }
    const per_partition_elem<T_size_t> &sizes() const { return sizes_; }

    T *operator[](std::size_t idx) { return elems_[idx]; }
    const T *operator[](std::size_t idx) const { return elems_[idx]; }

    T *mine() { return elems_[my_partition_id__]; }
    const T *mine() const { return elems_[my_partition_id__]; }

    void data_copy(T *data)
    {
        memcpy(elems_[my_partition_id__], data,
               sizes_[my_partition_id__] * sizeof(T));
    }

    T elem(int32_t i, int32_t j) { return elems_[i][j]; }
...
class PartitionStatistics {
   ...
   public:
    ...
    static Range<int64_t> vector_chunks_to_edge_subchunks(
        Range<int64_t> vector_chunks);
   ...
   public:
    ...
    {
        node_t deleted_version =
            (vid >> ((8 * sizeof(node_t)) - NUM_BITS_FOR_DELETED_VERSION)) &
            (DELETE_MASKING_LSBS);
        return deleted_version == 0 ? std::numeric_limits<node_t>::max()
                                    : deleted_version;
    }
...
class LocalStatistics {
   ...
   public:
    ...
    {
        ALWAYS_ASSERT(vid >= PartitionStatistics::my_first_internal_vid() &&
                      vid <= PartitionStatistics::my_last_internal_vid());
...
void ComputeStatistics(per_partition_array<T> &pa, int steps, int i)
{
    ...
    for (int _step = 0; _step < steps; _step++) {
        ...
        final_statistics_[i][_step].max =
            ComputeMax<T>(pa, _step, final_statistics_[i][_step].max_partition);
        final_statistics_[i][_step].min =
            ComputeMin<T>(pa, _step, final_statistics_[i][_step].min_partition);
        final_statistics_[i][_step].avr = ComputeAvr<T>(pa, _step);
        final_statistics_[i][_step].stddev = ComputeStddev<T>(pa, _step);
        final_statistics_[i][_step].balance = ComputeBalance<T>(
            final_statistics_[i][_step].sum, final_statistics_[i][_step].max);
...
void print_minor_result(per_partition_array<T> pa[], int size)
{
    ...
    LOG(INFO) << "============================================================="
                 "============================================================="
                 "=============";
    LOG(INFO) << "                                           <Minor Step Local "
                 "Statistics>";
    std::ostringstream _title;
    _title << std::left << std::setw(title_width) << std::setfill(separator)
           << "COMPUTE";
    _title << std::left << std::setw(title_width) << std::setfill(separator)
           << "VECTOR_REQ_NEXT";
    _title << std::left << std::setw(title_width) << std::setfill(separator)
    ...
    for (int j = 0; j < size; j++) {
        ...
        for (int i = 0; i < 5; i++) {
            _value << std::left << std::setw(value_width)
                   << std::setfill(separator)
                   << LocalStatistics::minor_timers_[i][j];
        }
...
void printResult_cout(int steps)
{
    ...
    std::cout << "============================================================="
                 ...
                 "================================================"
              << std::endl;
    std::cout << std::left << std::setw(step_width) << std::setfill(separator)
              << "step";
    std::cout << std::left << std::setw(title_width) << std::setfill(separator)
              << "SUPERSTEP";
    std::cout << std::left << std::setw(title_width) << std::setfill(separator)
              << "BEFORE_PAS";
    std::cout << std::left << std::setw(title_width) << std::setfill(separator)
    ...
    for (int _steps = 0; _steps < steps; _steps++) {
        std::cout << std::left << std::setw(step_width)
                  << std::setfill(separator) << _steps;
        for (int i = 0; i < N_PERF_METRICS; i++) {
            std::cout << std::left << std::setw(value_width)
                      << std::setfill(separator)
                      << final_statistics_[i][_steps].sum;
        }
        std::cout << std::endl;
        ...
        for (int i = 0; i < N_PERF_METRICS; i++) {
            std::cout << std::left << std::setw(value_width)
                      << std::setfill(separator)
                      << final_statistics_[i][_steps].max;
        }
        std::cout << std::endl;
        ...
        for (int i = 0; i < N_PERF_METRICS; i++) {
            std::cout << std::left << std::setw(value_width)
                      << std::setfill(separator)
                      << final_statistics_[i][_steps].min;
        }
        std::cout << std::endl;
        ...
        for (int i = 0; i < N_PERF_METRICS; i++) {
            std::cout << std::left << std::setw(value_width)
                      << std::setfill(separator)
                      << final_statistics_[i][_steps].avr;
        }
        std::cout << std::endl;
        ...
        for (int i = 0; i < N_PERF_METRICS; i++) {
            std::cout << std::left << std::setw(value_width)
                      << std::setfill(separator)
                      << final_statistics_[i][_steps].stddev;
        }
        std::cout << std::endl;
        ...
        for (int i = 0; i < N_PERF_METRICS; i++) {
            std::cout << std::left << std::setw(value_width)
                      << std::setfill(separator)
                      << final_statistics_[i][_steps].balance;
        }
        std::cout << std::endl;
    }
    std::cout << std::endl;
    ...
    std::cout << "                                                    < (MAX, "
              ...
              << std::endl;
    std::ostringstream _title;
    _title << std::left << std::setw(step_width) << std::setfill(separator)
           << "step";
    _title << std::left << std::setw(title_width) << std::setfill(separator)
           << "SUPERSTEP";
    _title << std::left << std::setw(title_width) << std::setfill(separator)
    ...
    for (int _steps = 0; _steps < steps; _steps++) {
        ...
        for (int i = 0; i < N_PERF_METRICS; i++) {
            ...
            _p << "(" << final_statistics_[i][_steps].max_partition << ", "
               << final_statistics_[i][_steps].min_partition << ")";
            std::cout << std::left << std::setw(value_width)
                      << std::setfill(separator) << _p.str().c_str();
        }
        std::cout << std::endl;
    }
    std::cout << "============================================================="
...
void printResult(per_partition_array<T> pa[], int steps)
{
    ...
    for (int _steps = 0; _steps < steps; _steps++) {
        std::ostringstream _line_sum, _line_max, _line_min, _line_avg,
            _line_balance, _line_stddev;

        _line_sum << std::left << std::setw(step_width)
                  << std::setfill(separator) << _steps;
        for (int i = 0; i < N_PERF_METRICS; i++) {
            _line_sum << std::left << std::setw(value_width)
                      << std::setfill(separator)
                      << final_statistics_[i][_steps].sum;
        }
        LOG(INFO) << _line_sum.str().c_str();

        _line_max << std::left << std::setw(step_width)
                  << std::setfill(separator) << _steps;
        for (int i = 0; i < N_PERF_METRICS; i++) {
            _line_max << std::left << std::setw(value_width)
                      << std::setfill(separator)
                      << final_statistics_[i][_steps].max;
        }
        LOG(INFO) << _line_max.str().c_str();

        _line_min << std::left << std::setw(step_width)
                  << std::setfill(separator) << _steps;
        for (int i = 0; i < N_PERF_METRICS; i++) {
            _line_min << std::left << std::setw(value_width)
                      << std::setfill(separator)
                      << final_statistics_[i][_steps].min;
        }
        LOG(INFO) << _line_min.str().c_str();

        _line_avg << std::left << std::setw(step_width)
                  << std::setfill(separator) << _steps;
        for (int i = 0; i < N_PERF_METRICS; i++) {
            _line_avg << std::left << std::setw(value_width)
                      << std::setfill(separator)
                      << final_statistics_[i][_steps].avr;
        }
        LOG(INFO) << _line_avg.str().c_str();

        _line_stddev << std::left << std::setw(step_width)
                     << std::setfill(separator) << _steps;
        for (int i = 0; i < N_PERF_METRICS; i++) {
            _line_stddev << std::left << std::setw(value_width)
                         << std::setfill(separator)
                         << final_statistics_[i][_steps].stddev;
        }
        LOG(INFO) << _line_stddev.str().c_str();

        _line_balance << std::left << std::setw(step_width)
                      << std::setfill(separator) << _steps;
        for (int i = 0; i < N_PERF_METRICS; i++) {
            _line_balance << std::left << std::setw(value_width)
                          << std::setfill(separator)
                          << final_statistics_[i][_steps].balance;
        }
    ...
    for (int _steps = 0; _steps < steps; _steps++) {
        ...
        _mm << std::left << std::setw(step_width) << std::setfill(separator)
            << _steps;
        for (int i = 0; i < N_PERF_METRICS; i++) {
            std::ostringstream _vv;
            _vv << "(" << final_statistics_[i][_steps].max_partition << ", "
                << final_statistics_[i][_steps].min_partition << ")";
            _mm << std::left << std::setw(value_width)
                << std::setfill(separator) << _vv.str();
        }
    ...
    for (int _steps = 0; _steps < steps; _steps++) {
        ...
        _line << std::left << std::setw(step_width) << std::setfill(separator)
              << _steps;

        for (int i = 0; i < N_PERF_METRICS; i++) {
            _line << std::left << std::setw(value_width)
                  << std::setfill(separator)
                  << LocalStatistics::timers_[i][_steps];
        }
