...
class GBVersionedArrayBase {

    public:
        GBVersionedArrayBase() {}
        /*VersionedArrayBase() : use_main_(false) {
            has_future_job = false;
        }*/
       
        virtual GBVersionedArrayBase<T, op>* GetRef() = 0;
        virtual bool IsCreated() = 0;

        // Initialize & Copy functions
        virtual void ComputeMemoryUsage() = 0;
        virtual void InitReadOnly(GBVersionedArrayBase<T, op>& o, bool maintain_prev_ss=false) = 0;
        ...
        virtual void CopyBuffer(GBVersionedArrayBase<T, op>* prev) = 0;
        virtual void CopyMainBuffer() = 0;
        virtual void CopyMainBufferFromRef(GBVersionedArrayBase<T, op>* prev) = 0;
        virtual void CopyPrevSSBuffer() {}
        virtual void PrepareNextSSBuffer() {}
        virtual void InitializeAfterAdvanceUpdateVersion() {}


        // Read & Construct From Disk, Merge functions
        virtual void ReadUpdatesAndConstructWithoutFlush(int i, int j, bool construct_next, bool construct_prev, bool mark_changed, GBVersionedArrayBase<T, op>* prev=NULL) = 0;
        virtual void MergeWithNext(Version from, Version to) = 0;        // OV
        virtual void MergeWithNext(Version from, Version to, GBVersionedArrayBase<T, op>* prev) = 0;   // IV
        ...
        virtual void _SetBufPtr(int superstep_version) = 0;
        ...
        static void call_ConstructNextAsync(GBVersionedArrayBase<T, op>* va, Version from, Version to) {
            return;
        ...
        void InitializeFlags() {
            ...
            changed_flags[0].Init(buf_length);
            changed_flags[1].Init(buf_length);
        ...
        void FreeFlags() {
            dirty_flags.Close();
            discard_flags.Close();
            changed_flags[0].Close();
            changed_flags[1].Close();
        ...
        void Init(const std::string& file_name, std::size_t num_entries, bool open, bool use_main, bool is_ov=false, bool maintain_prev_ss=false, bool advance_superstep=true, bool construct_next_ss_array_async=true) {
            ...
            num_max_discard_deltas_in_a_snapshot = 3;   // XXX
        ...
        void __ConstructAndFlushFromTo(Version from, Version to, GBVersionedArrayBase<T, op>* prev, bool construct_prev, bool flush) {
            // Construct --> Swap Buffer  
            ...
            if (to.GetSuperstepVersion() >= 0 && !use_main_)
                _SetBufPtr(to.GetSuperstepVersion());
        }

        void _ConstructAndFlushFromTo(Version from, Version to, GBVersionedArrayBase<T, op>* prev, bool construct_prev, bool flush) {
            if (!use_main_) return;
        ...
        void _ConstructWithNextBufferAndFlushFromTo(Version from, Version to, GBVersionedArrayBase<T, op>* prev, bool construct_prev, bool flush) {
#ifdef INCREMENTAL_LOGGING
...
//#endif
        ...
        void FlushAll_(Version to, bool apply_hybrid_model=true) {
            //system_fprintf(0, stdout, "FlushAll %s\t%ld\t%ld, %s %s\n", this->array_path.c_str(), to.GetUpdateVersion(), to.GetSuperstepVersion(), use_main_ ? "true" : "false", constructed_already_ ? "true" : "false");
            if (!use_main_) return;
...
#pragma omp parallel num_threads(UserArguments::NUM_THREADS)
        ...
        void ConstructAndFlush(Range<int64_t> idx_range, int update_version, int superstep_version, bool construct_prev=false, GBVersionedArrayBase<T, op>* prev=NULL, bool flush=false) {
            ...
            //fprintf(stdout, "[%ld] (%d, %d, %d) ConstructAndFlush %s cur_ver [%d, %d] -> to [%d, %d] advance_superstep_ %s constructed_already_ %s flush %s\n", PartitionStatistics::my_machine_id(), UserArguments::UPDATE_VERSION, UserArguments::SUPERSTEP, UserArguments::INC_STEP, array_name.c_str(), current_version.GetUpdateVersion(), current_version.GetSuperstepVersion(), update_version, superstep_version, advance_superstep_ ? "true" : "false", constructed_already_ ? "true" : "false", flush ? "true" : "false");
            ...
            if (UserArguments::INC_STEP == -1 && !constructed_already_) {
                ...
                if (current_version.GetSuperstepVersion() < superstep_version) {
                    ...
                    //fprintf(stdout, "%s SetSuperstepVersion %d\n", array_name.c_str(), superstep_version);
                }
                return;
            }
        ...
        inline void MarkChangedAll() { changed_flags[changed_toggle_].SetAll(); }
        inline void MarkChanged(int64_t idx) { changed_flags[changed_toggle_].Set_Atomic(idx); }
        inline void MarkChangedUnsafe(int64_t idx) { changed_flags[changed_toggle_].Set(idx); }
        inline void ClearChanged(int64_t idx) { changed_flags[changed_toggle_].Clear_Atomic(idx); }
        inline void ClearAllChanged() { 
            changed_flags[0].ClearAll(); 
            changed_flags[1].ClearAll(); 
        }
        inline bool IsChanged(int64_t idx) { return changed_flags[changed_toggle_].Get(idx); }
        ...
        TwoLevelBitMap<int64_t>* GetChangedFlags() { return &changed_flags[changed_toggle_]; }
    ...
    public:
        ...
        TwoLevelBitMap<int64_t> changed_flags[2];
