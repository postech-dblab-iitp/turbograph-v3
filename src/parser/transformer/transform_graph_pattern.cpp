...
std::unique_ptr<NodePattern> Transformer::transformNodePattern(CypherParser::OC_NodePatternContext& ctx) {
    ...
    auto properties = std::vector<ParsedPropertyKeyValue>{};
    if (ctx.kU_Properties()) {
        properties = transformProperties(*ctx.kU_Properties());
    }
    return std::make_unique<NodePattern>(std::move(variable), std::move(nodeLabels), std::move(properties));
}
...
std::unique_ptr<RelPattern> Transformer::transformRelationshipPattern(
    ...
    return std::make_unique<RelPattern>(variable, relTypes, relType, arrowDirection, std::move(properties),
        std::move(recursiveInfo));
}

std::vector<ParsedPropertyKeyValue> Transformer::transformProperties(
    CypherParser::KU_PropertiesContext& ctx) {
    std::vector<ParsedPropertyKeyValue> result;
...
std::vector<EdgeType> Transformer::transformRelTypes(
...
std::vector<NodeLabel> Transformer::transformNodeLabels(CypherParser::OC_NodeLabelsContext& ctx) {
...
NodeLabel Transformer::transformNodeLabel(CypherParser::OC_NodeLabelContext& ctx) {
    return transformLabelName(*ctx.oC_LabelName());
}

NodeLabel Transformer::transformLabelName(CypherParser::OC_LabelNameContext& ctx) {
    return transformSchemaName(*ctx.oC_SchemaName());
}

EdgeType Transformer::transformRelTypeName(CypherParser::OC_RelTypeNameContext& ctx) {
    return transformSchemaName(*ctx.oC_SchemaName());
}
