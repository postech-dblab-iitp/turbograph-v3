...
std::unique_ptr<CypherStatement> Transformer::transformQuery(CypherParser::OC_QueryContext& ctx) {
    return transformRegularQuery(*ctx.oC_RegularQuery());
}

std::unique_ptr<CypherStatement> Transformer::transformRegularQuery(
...
std::unique_ptr<SingleQuery> Transformer::transformSingleQuery(CypherParser::OC_SingleQueryContext& ctx) {
    auto singleQuery =
...
std::unique_ptr<SingleQuery> Transformer::transformSinglePartQuery(CypherParser::OC_SinglePartQueryContext& ctx) {
    ...
    for (auto& updatingClause : ctx.oC_UpdatingClause()) {
        throw ParserException("Updating clause is not supported.");
    }
    if (ctx.oC_Return()) {
