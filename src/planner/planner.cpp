...
Planner::Planner(PlannerConfig config, MDProviderType mdp_type,
    ...
    : config(config),
      mdp_type(mdp_type),
      context(context),
      memory_mdp_filepath(memory_mdp_path)
{
    ...
    id_col_cname = GPOS_NEW(memory_pool)
        CName(GPOS_NEW(memory_pool) CWStringConst(w_id_col_name),
              true /*fOwnsMemory*/);
    sid_col_cname = GPOS_NEW(memory_pool)
        CName(GPOS_NEW(memory_pool) CWStringConst(w_sid_col_name),
              true /*fOwnsMemory*/);
                                          ...
                                          CExpression *logical_plan)
{
    ...
    result = GPOS_NEW(mp) CQueryContext(
        mp, logical_plan, prpp, colref_array, pdrgpmdname,
        true /*fDeriveStats*/,
        config.JOIN_ORDER_TYPE == PlannerConfig::JOIN_ORDER_GREEDY_SEARCH);
...
CMDProviderMemory *Planner::_orcaGetProviderMemory()
{
    ...
    CMemoryPool *mp = nullptr;
    {
        ...
        provider =
            new (mp, __FILE__, __LINE__) CMDProviderMemory(mp, md_path.c_str());
        // detach safety
        (void)amp.Detach();
    }
...
void Planner::_orcaSetTraceFlags()
{
    ...
    if (config.INDEX_JOIN_ONLY) {
        ...
        CBitSet *hash_index_join_bitset = CXform::PbsHashIndexJoinXforms(mp);
        traceflag_bitset->Union(hash_index_join_bitset);
        hash_index_join_bitset->Release();
    }
    else if (config.DISABLE_MERGE_JOIN) {
    ...
    SetTraceflags(mp, traceflag_bitset, &enabled_trace_flags,
...
gpdbcost::CCostModelGPDB *Planner::_orcaGetCostModel(CMemoryPool *mp)
{
    ...
    m_cost_model_params->SetParam(
        39, 1.0, 1.0,
        1.0);  // single machine cost - may need to make this alive
    ...
    m_cost_model_params->SetParam(15, 10000000.0, 10000000.0,
                                  10000000.0);  // redistribute cost
    m_cost_model_params->SetParam(16, 10000000.0, 10000000.0,
                                  10000000.0);  // redistribute cs
    m_cost_model_params->SetParam(17, 10000000.0, 10000000.0,
                                  10000000.0);  // broadcast cost
    m_cost_model_params->SetParam(18, 10000000.0, 10000000.0,
                                  10000000.0);  // broadcast cost

    gpdbcost::CCostModelGPDB *pcm =
        GPOS_NEW(mp) gpdbcost::CCostModelGPDB(mp, 1, m_cost_model_params);  // one segment
    D_ASSERT(pcm != nullptr);

    return pcm;
}

void Planner::_orcaSetOptCtxt(CMemoryPool *mp, CMDAccessor *mda,
                              gpdbcost::CCostModelGPDB *pcm)
{
    ...
    COptimizerConfig *optimizer_config = GPOS_NEW(mp) COptimizerConfig(
        GPOS_NEW(mp) CEnumeratorConfig(mp, 0 /*plan_id*/, 100 /*ullSamples*/,
                                       CDouble(1000.0)),
        ...
        for (auto &col : output_columns) {
            ...
            if (planner->property_col_to_output_col_names_mapping.find(col) !=
                ...
                output_names.push_back(
                    planner->property_col_to_output_col_names_mapping[col]);
                continue;
            }
