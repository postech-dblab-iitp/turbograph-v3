 ...
 public:
  ...
  int operator()(T value) {
    if (!int_checker<std::numeric_limits<T>::is_signed>::fits_in_int(value))
      FMT_THROW(s62::Exception("number is too big"));
    return (std::max)(static_cast<int>(value), 0);
  }

  template <typename T, FMT_ENABLE_IF(!std::is_integral<T>::value)>
  int operator()(T) {
    FMT_THROW(s62::Exception("precision is not integer"));
    return 0;
  }
};
...
template <typename Char> class printf_width_handler {
 ...
 public:
  ...
  unsigned operator()(T value) {
    ...
    if (width > int_max) FMT_THROW(s62::Exception("number is too big"));
    return static_cast<unsigned>(width);
  }

  template <typename T, FMT_ENABLE_IF(!std::is_integral<T>::value)>
  unsigned operator()(T) {
    FMT_THROW(s62::Exception("width is not integer"));
    return 0;
  }
};
...
OutputIt basic_printf_context<OutputIt, Char>::format() {
  ...
  while (it != end) {
    ...
    // Parse type.
    if (it == end) FMT_THROW(s62::Exception("invalid format string"));
