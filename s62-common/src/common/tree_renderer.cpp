...
string TreeRenderer::ToString(const CypherPhysicalOperator &op) {
	std::stringstream ss;
	Render(op, ss);
	return ss.str();
}
...
string TreeRenderer::ToString(const CypherPipeline &op) {
	std::stringstream ss;
	Render(op, ss);
	return ss.str();
}
...
void TreeRenderer::Render(const CypherPhysicalOperator &op, std::ostream &ss) {
	auto tree = CreateTree(op);
	ToStream(*tree, ss);
}
...
void TreeRenderer::Render(const CypherPipeline &op, std::ostream &ss) {
	auto tree = CreateTree(op);
	ToStream(*tree, ss);
}
...
template <>
bool TreeChildrenIterator::HasChildren(const CypherPhysicalOperator &op) {
...
template <>
void TreeChildrenIterator::Iterate(const CypherPhysicalOperator &op,
                                   const std::function<void(const CypherPhysicalOperator &child)> &callback) {
	for (auto &child : op.children) {
		callback(*child);
	}
	// if (op.type == PhysicalOperatorType::DELIM_JOIN) {
...
struct PipelineRenderNode {
	explicit PipelineRenderNode(CypherPhysicalOperator &op) : op(op) {
	}
...
unique_ptr<RenderTreeNode> TreeRenderer::CreateNode(const CypherPhysicalOperator &op) {
	return CreateRenderNode(op.ToString(), op.ParamsToString());
}
...
unique_ptr<RenderTreeNode> TreeRenderer::CreateNode(const QueryProfiler::TreeNode &op) {
	...
	string timing = StringUtil::Format("%.3f", op.info.time * 1000.0);
	result->extra_text += "\n(" + timing + "ms)";
	if (config.detailed) {
...
unique_ptr<RenderTree> TreeRenderer::CreateTree(const CypherPhysicalOperator &op) {
	return CreateRenderTree<CypherPhysicalOperator>(op);
}
...
unique_ptr<RenderTree> TreeRenderer::CreateTree(const CypherPipeline &op) {
