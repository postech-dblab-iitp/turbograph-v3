...
class ExtentIterator {
   public:
    ...
    void Initialize(ClientContext &context, vector<LogicalType> &target_types_,
                    vector<idx_t> &target_idxs_, ExtentID target_eid);
    void Initialize(ClientContext &context,
    ...
    int RequestNewIO(ClientContext &context, vector<LogicalType> &target_types_,
                     vector<idx_t> &target_idxs_, ExtentID target_eid,
                     ExtentID &evicted_eid);
    bool RequestNextIO(ClientContext &context, DataChunk &output,
                       ExtentID &output_eid, bool is_output_chunk_initialized);
    void Rewind();

    /* no filter pushdown */
    bool GetNextExtent(ClientContext &context, DataChunk &output,
                       ExtentID &output_eid,
                       size_t scan_size = EXEC_ENGINE_VECTOR_SIZE,
                       bool is_output_chunk_initialized = true);
    bool GetNextExtent(ClientContext &context, DataChunk &output,
                       ExtentID &output_eid, vector<idx_t> &output_column_idxs,
                       size_t scan_size = EXEC_ENGINE_VECTOR_SIZE,
                       bool is_output_chunk_initialized = true);
    ...
    bool GetNextExtent(ClientContext &context, DataChunk &output,
                       ...
                       int64_t &filterKeyColIdx, Value &filterValue,
    ...
    bool GetNextExtent(ClientContext &context, DataChunk &output,
                       FilteredChunkBuffer &output_buffer, ExtentID &output_eid,
                       int64_t &filterKeyColIdx, Value &lfilterValue,
                       Value &rfilterValue, bool l_inclusive, bool r_inclusive,
                       vector<idx_t> &output_column_idxs,
                       vector<s62::LogicalType> &scanSchema,
                       size_t scan_size = EXEC_ENGINE_VECTOR_SIZE,
                       bool is_output_chunk_initialized = true);
    ...
    bool GetNextExtent(ClientContext &context, DataChunk &output,
                       ...
                       bool is_output_chunk_initialized = true);
    bool GetNextExtentInRowFormat(ClientContext &context, DataChunk &output,
                                  ExtentID &output_eid, ExtentID target_eid,
                                  DataChunk &input, idx_t nodeColIdx,
                                  const vector<uint32_t> &output_column_idxs,
    ...
    bool GetNextExtent(ClientContext &context, DataChunk &output,
                       ...
                       const vector<uint32_t> &output_column_idxs,
                       vector<uint32_t> &target_seqnos,
                       vector<idx_t> &cols_to_include, idx_t &output_seqno,
                       bool is_output_chunk_initialized = true);
    bool GetExtent(data_ptr_t &chunk_ptr, int target_toggle,
                   bool is_initialized);
    ...
    bool IsRewinded() { return is_rewinded; }

    bool IsInitialized() { return is_initialized; }

    /* Filter Buffering */
    inline void enableFilterBuffering() { is_filter_buffering_enabled = true; }
   ...
   private:
    ...
    void evalEQPredicateSIMD(Vector &column_vec, size_t data_len,
                             std::unique_ptr<TFilter> &filter,
                             idx_t scan_start_offset, idx_t scan_end_offset,
                             vector<idx_t> &matched_row_idxs);

    idx_t findColumnIdx(ChunkDefinitionID filter_cdf_id);
    ChunkDefinitionID getFilterCDFID(ExtentID output_eid,
                                     int64_t filterKeyColIdx);
    void requestIOForDoubleBuffering(ClientContext &context);
    void requestFinalizeIO();

    bool getScanRange(size_t scan_size, idx_t &scan_start_offset,
                      idx_t &scan_end_offset);
    bool getScanRange(size_t scan_size, idx_t idx_in_extent,
                      idx_t &scan_start_offset, idx_t &scan_end_offset);
    bool getScanRange(ClientContext &context, ChunkDefinitionID filter_cdf_id,
                      Value &filterValue, size_t scan_size,
                      idx_t &scan_start_offset, idx_t &scan_end_offset);
    bool getScanRange(ClientContext &context, ChunkDefinitionID filter_cdf_id,
                      s62::Value &l_filterValue, s62::Value &r_filterValue,
                      bool l_inclusive, bool r_inclusive, size_t scan_size,
                      idx_t &scan_start_offset, idx_t &scan_end_offset);
    void selVectorToRowIdxs(SelectionVector &sel, size_t sel_size,
                            vector<idx_t> &row_idxs, idx_t offset);
    void getValidOutputMask(vector<idx_t> &output_column_idxs,
                            vector<bool> &valid_output_mask);
    void findMatchedRowsEQFilter(CompressionHeader &comp_header, idx_t col_idx,
                                 idx_t scan_start_offset, idx_t scan_end_offset,
                                 Value &filterValue,
                                 vector<idx_t> &matched_row_idxs);
    void findMatchedRowsRangeFilter(CompressionHeader &comp_header,
                                    idx_t col_idx, idx_t scan_start_offset,
                                    idx_t scan_end_offset, Value &l_filterValue,
                                    Value &r_filterValue, bool l_inclusive,
                                    bool r_inclusive,
                                    vector<idx_t> &matched_row_idxs);
    void referenceRows(DataChunk &output, ExtentID output_eid, size_t scan_size,
                       vector<idx_t> &output_column_idxs,
                       idx_t scan_begin_offset, idx_t scan_end_offset);
    bool copyMatchedRowsToBuffer(CompressionHeader &comp_header,
                                 vector<idx_t> &matched_row_idxs,
                                 vector<idx_t> &output_column_idxs,
                                 ExtentID &output_eid,
                                 FilteredChunkBuffer &output);
    void copyMatchedRows(CompressionHeader &comp_header,
                         vector<idx_t> &matched_row_idxs,
                         vector<idx_t> &output_column_idxs,
                         ExtentID &output_eid, DataChunk &output);
    bool inclusiveAwareRangePredicateCheck(Value &l_filterValue,
                                           Value &r_filterValue,
    ...
    inline bool doFilterBuffer(size_t scan_size, size_t num_filtered_tuples)
    {
        if (!is_filter_buffering_enabled) {
            ...
            return (double)num_filtered_tuples / scan_size <
                   FILTER_BUFFERING_THRESHOLD;
        }
    }

    inline size_t getNumReferencedRows(size_t scan_size)
    {
        size_t remain_data_size = num_tuples_in_current_extent[toggle] -
                                  (current_idx_in_this_extent * scan_size);
        return std::min((size_t)scan_size, remain_data_size);
    }

    void sliceFilteredRows(DataChunk &input, DataChunk &output,
