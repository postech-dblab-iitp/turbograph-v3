...
const BufPtrAdjIdxPair INVALID_PTR_ADJ_IDX_PAIR =
    std::make_pair<idx_t, data_ptr_t>(-2, nullptr);
...
class AdjacencyListIterator {
   public:
    AdjacencyListIterator()
    {
        ...
        eid_to_bufptr_idx_map->resize(INITIAL_EXTENT_ID_SPACE,
                                      INVALID_PTR_ADJ_IDX_PAIR);
    }
    ~AdjacencyListIterator() {}
    AdjacencyListIterator(
        std::shared_ptr<ExtentIterator> share_ext_it,
        std::shared_ptr<vector<BufPtrAdjIdxPair>> share_eid_to_bufptr_idx_map)
    {
        ext_it = share_ext_it;
        eid_to_bufptr_idx_map = share_eid_to_bufptr_idx_map;
    }
    bool Initialize(ClientContext &context, int adjColIdx, ExtentID target_eid,
                    bool is_fwd);
    void getAdjListPtr(uint64_t vid, ExtentID target_eid, uint64_t **start_ptr,
                       uint64_t **end_ptr, bool is_initialized);
    int requestNewAdjList(ClientContext &context, int adjColIdx,
                          ExtentID target_eid, bool is_fwd);
...
class DFSIterator {
   public:
    ...
    void initialize(ClientContext &context, uint64_t src_id,
                    uint64_t adj_col_idx);
    bool getNextEdge(ClientContext &context, int lv, uint64_t &tgt,
                     uint64_t &edge);
    void reduceLevel();

   private:
    void changeLevel(ClientContext &context, bool traverse_child,
                     uint64_t src_id = 0);
...
class ShortestPathIterator {
   public:
    ...
    void initialize(ClientContext &context, NodeID src_id, NodeID tgt_id,
                    uint64_t adj_col_idx, Level lower_bound, Level upper_bound);
    bool getShortestPath(ClientContext &context, std::vector<EdgeID> &edges,
   ...
   private:
    ...
    Level lowerBound;
    Level upperBound;
    std::unordered_map<NodeID, std::pair<NodeID, EdgeID>>
        predecessor;  // Node ID mapped to a pair of its predecessor and the connecting edge ID
    std::shared_ptr<AdjacencyListIterator> adjlist_iterator;
    std::shared_ptr<ExtentIterator> ext_it = nullptr;
    std::shared_ptr<vector<BufPtrAdjIdxPair>> eid_to_bufptr_idx_map;

    bool enqueueNeighbors(ClientContext &context, NodeID node_id,
                          Level node_level,
...
class ShortestPathAdvancedIterator {
   public:
    ...
    void initialize(ClientContext &context, NodeID src_id, NodeID tgt_id,
                    uint64_t adj_col_idx_forward, uint64_t adj_col_idx_backward,
   ...
   private:
    ...
    Level lower_bound;
    Level upper_bound;
    std::unordered_map<NodeID, std::pair<NodeID, EdgeID>, NodeIDHasher>
        predecessor_forward;
    std::unordered_map<NodeID, std::pair<NodeID, EdgeID>, NodeIDHasher>
        predecessor_backward;
...
class AllShortestPathIterator {
   public:
    ...
    void initialize(ClientContext &context, NodeID src_id, NodeID tgt_id,
                    uint64_t adj_col_idx_forward, uint64_t adj_col_idx_backward,
                    Level lower_bound, Level upper_bound);
    bool getAllShortestPaths(ClientContext &context,
                             std::vector<std::vector<EdgeID>> &all_edges,
                             std::vector<std::vector<NodeID>> &all_nodes);
   ...
   private:
    ...
    std::unordered_map<NodeID, std::vector<std::pair<NodeID, EdgeID>>,
                       NodeIDHasher>
        predecessor_forward;
    std::unordered_map<NodeID, std::vector<std::pair<NodeID, EdgeID>>,
                       NodeIDHasher>
