...
void pin_obj(cache_t* cache, obj_id_t obj_id) {
...
void unpin_obj(cache_t* cache, obj_id_t obj_id) {
  cache_obj_t *obj = hashtable_find_obj_id(cache->hashtable, obj_id);
  if (obj == NULL) {
    return;
  }
...
bool is_pinned(cache_t* cache, obj_id_t obj_id) {
  cache_obj_t *obj = hashtable_find_obj_id(cache->hashtable, obj_id);
  if (obj == NULL) {
    return false;
  }
  if(obj->pin_count < 0) {
    printf("obj_id: %lu, pin count %d is negative!\n", obj_id, obj->pin_count);
    abort();
  }
  return obj->pin_count > 0;
}
