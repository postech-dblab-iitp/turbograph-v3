...
static cache_obj_t *LFU_find(cache_t *cache, const request_t *req, const bool update_cache);
...
static obj_id_t LFU_evict_return_obj(cache_t *cache, const request_t *req);
...
cache_t *LFU_init(const common_cache_params_t ccache_params, const char *cache_specific_params) {
  ...
  cache->evict_return_obj = LFU_evict_return_obj;
 ...
 */
static bool LFU_get(cache_t *cache, const request_t *req) { return cache_get_base(cache, req); }
...
static cache_obj_t *LFU_insert(cache_t *cache, const request_t *req) {
  ...
  append_obj_to_tail(&freq_one_node->first_obj, &freq_one_node->last_obj, cache_obj);
...
static obj_id_t LFU_evict_return_obj(cache_t *cache, const request_t *req) {
  ...
  obj_id_t evicted_obj_id = -1;  // Initialize to an invalid ID
  ...
  if (obj_to_evict == NULL) {
    for (uint64_t freq = params->min_freq + 1; freq <= params->max_freq; freq++) {
      ...
      if (node != NULL && node->n_obj > 0) {
        ...
        if (obj_to_evict != NULL) break;
      }
    }
  }

  // If we found an object to evict
  if (obj_to_evict != NULL) {
    evicted_obj_id = obj_to_evict->obj_id;
    min_freq_node->n_obj--;
