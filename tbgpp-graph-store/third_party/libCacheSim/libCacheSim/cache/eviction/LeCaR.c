...
static void LeCaR_parse_params(cache_t *cache, const char *cache_specific_params);
static void LeCaR_free(cache_t *cache);
static bool LeCaR_get(cache_t *cache, const request_t *req);
static cache_obj_t *LeCaR_find(cache_t *cache, const request_t *req, const bool update_cache);
...
static obj_id_t LeCaR_evict_return_obj(cache_t *cache, const request_t *req);
...
cache_t *LeCaR_init(const common_cache_params_t ccache_params, const char *cache_specific_params) {
  ...
  cache->evict_return_obj = LeCaR_evict_return_obj;
  ...
  freq_node->freq = 1;
  params->freq_map = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, (GDestroyNotify)free_freq_node);
  g_hash_table_insert(params->freq_map, GSIZE_TO_POINTER(1), freq_node);

  if (!params->update_weight) {
    snprintf(cache->cache_name, CACHE_NAME_ARRAY_LEN, "LeCaR-%.4lflru", params->w_lru);
  }
 ...
 */
static cache_obj_t *LeCaR_find(cache_t *cache, const request_t *req, bool update_cache) {
  LeCaR_params_t *params = (LeCaR_params_t *)(cache->eviction_params);
  ...
  if (cache_obj->LeCaR.is_ghost) {
    if (cache_obj->LeCaR.evict_expert == 1) {
      ...
      remove_obj_from_list(&params->ghost_lfu_head, &params->ghost_lfu_tail, cache_obj);
...
void LeCaR_evict(cache_t *cache, const request_t *req) {
  ...
  if (obj_to_evict->LeCaR.evict_expert == 1) {
    prepend_obj_to_head(&params->ghost_lru_head, &params->ghost_lru_tail, obj_to_evict);
    ...
    while (params->lru_g_occupied_byte > cache->cache_size / 2) {
      ...
      params->lru_g_occupied_byte -= (ghost_to_evict->obj_size + cache->obj_md_size);
      remove_obj_from_list(&params->ghost_lru_head, &params->ghost_lru_tail, ghost_to_evict);
      hashtable_delete(cache->hashtable, ghost_to_evict);
    }
  } else if (obj_to_evict->LeCaR.evict_expert == 2) {
    prepend_obj_to_head(&params->ghost_lfu_head, &params->ghost_lfu_tail, obj_to_evict);
    ...
    while (params->lfu_g_occupied_byte > cache->cache_size / 2) {
      ...
      DEBUG_ASSERT(ghost_to_evict != NULL);
      params->lfu_g_occupied_byte -= (ghost_to_evict->obj_size + cache->obj_md_size);
      remove_obj_from_list(&params->ghost_lfu_head, &params->ghost_lfu_tail, ghost_to_evict);
      hashtable_delete(cache->hashtable, ghost_to_evict);
    }
  } else {
...
obj_id_t LeCaR_evict_return_obj(cache_t *cache, const request_t *req) {
  LeCaR_params_t *params = (LeCaR_params_t *)(cache->eviction_params);

  cache_obj_t *lru_candidate = cache->to_evict_candidate = params->q_tail;
  // Find the first unpinned object from the tail
  while (lru_candidate != NULL && is_pinned(cache, lru_candidate->obj_id)) {
    lru_candidate = lru_candidate->queue.prev;
  }

  cache_obj_t *lfu_candidate = NULL;
  freq_node_t *min_freq_node = get_min_freq_node(params);

  // Find the first unpinned object in the min frequency node
  cache_obj_t *curr_obj = min_freq_node->first_obj;
  while (curr_obj != NULL) {
    if (!is_pinned(cache, curr_obj->obj_id)) {
      lfu_candidate = curr_obj;
      break;
    }
    else {
      // printf("object %lu is pinned\n", curr_obj->obj_id);
    }
    curr_obj = curr_obj->LeCaR.lfu_next;
  }
  if(lfu_candidate) {
    // printf("found lfu candidate from min freq node, whose lfu freq is %d\n", lfu_candidate->LeCaR.freq);
  }

  // If all objects in the current min frequency node are pinned,
  // search in higher frequency nodes
  if (lfu_candidate == NULL) {
    for (int64_t freq = params->min_freq + 1; freq <= params->max_freq; freq++) {
      ...
      if(node == NULL || node->n_obj == 0) {
        // printf("freq node %lu is empty, null: %d\n", freq, node == NULL);
      }
      else {
        // printf("freq node %lu is not empty, n_obj: %d\n", freq, node->n_obj);
      }
      if (node != NULL && node->n_obj > 0) {
        curr_obj = node->first_obj;
        while (curr_obj != NULL) {
          if (!is_pinned(cache, curr_obj->obj_id)) {
            // printf("found lfu candidate from freq node %lu, whose lfu freq is %d\n", freq, curr_obj->LeCaR.freq);
            lfu_candidate = curr_obj;
            min_freq_node = node;
            break;
          }
        ...
        if (lfu_candidate != NULL) break;
      }
    }
  }
  
  if(lru_candidate == NULL || lfu_candidate == NULL) {
    if(lru_candidate != NULL || lfu_candidate != NULL) {
      abort();
    }
    return -1;
...
// ****                                                               ****
// ***********************************************************************
static const char *LeCaR_current_params(cache_t *cache, LeCaR_params_t *params) {
  static __thread char params_str[128];
  snprintf(params_str, 128, "update-weight=%d,lru-weight=%.lf", params->update_weight, params->w_lru);
...
static inline void update_LFU_min_freq(LeCaR_params_t *params) {
  ...
  VVERBOSE("update LFU min freq from %u to %u\n", (unsigned)old_min_freq, (unsigned)params->min_freq);
...
static inline void remove_obj_from_freq_node(LeCaR_params_t *params, cache_obj_t *cache_obj) {
  ...
  if (cache_obj == freq_node->first_obj) {
    ...
    if (cache_obj->LeCaR.lfu_next != NULL) ((cache_obj_t *)(cache_obj->LeCaR.lfu_next))->LeCaR.lfu_prev = NULL;
  }
  ...
  if (cache_obj == freq_node->last_obj) {
    ...
    if (cache_obj->LeCaR.lfu_prev != NULL) ((cache_obj_t *)(cache_obj->LeCaR.lfu_prev))->LeCaR.lfu_next = NULL;
  }
...
static inline void insert_obj_info_freq_node(LeCaR_params_t *params, cache_obj_t *cache_obj) {
  ...
  freq_node_t *new_node = g_hash_table_lookup(params->freq_map, GSIZE_TO_POINTER(cache_obj->LeCaR.freq));
  if (new_node == NULL) {
...
static void verify_ghost_lru_integrity(cache_t *cache, LeCaR_params_t *params) {
  ...
  VVVERBOSE(
      ...
      params->ghost_lru_head, params->ghost_lru_tail, ghost_entry_size, params->lru_g_occupied_byte);
