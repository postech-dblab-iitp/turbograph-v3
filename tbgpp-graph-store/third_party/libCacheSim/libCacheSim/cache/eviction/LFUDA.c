...
static cache_obj_t *LFUDA_find(cache_t *cache, const request_t *req, const bool update_cache);
...
static obj_id_t LFUDA_evict_return_obj(cache_t *cache, const request_t *req);
...
cache_t *LFUDA_init(const common_cache_params_t ccache_params, const char *cache_specific_params) {
  ...
  cache->evict_return_obj = LFUDA_evict_return_obj;
 ...
 */
static bool LFUDA_get(cache_t *cache, const request_t *req) { return cache_get_base(cache, req); }
...
static void LFUDA_evict(cache_t *cache, const request_t *req) {
  ...
  min_freq_node->n_obj--;
  remove_obj_from_list(&min_freq_node->first_obj, &min_freq_node->last_obj, obj_to_evict);
...
/**
 * @brief evict an object from the cache considering pin status and return its ID
 *
 * This function follows the logic of `LFUDA_evict` but skips pinned objects and
 * returns the ID of the evicted object.
 *
 * @param cache The cache structure
 * @param req The request structure (not used in this function)
 * @return The ID of the evicted object, or -1 if no suitable object is found
 */
static obj_id_t LFUDA_evict_return_obj(cache_t *cache, const request_t *req) {
  LFUDA_params_t *params = (LFUDA_params_t *)(cache->eviction_params);

  freq_node_t *curr_freq_node = get_min_freq_node(params);
  cache_obj_t *obj_to_evict = NULL;

  // Iterate through all frequency nodes if necessary
  while (curr_freq_node != NULL) {
    cache_obj_t *curr_obj = curr_freq_node->first_obj;

    // Find the first unpinned object in the current frequency node
    while (curr_obj != NULL) {
      if (!is_pinned(cache, curr_obj->obj_id)) {
        obj_to_evict = curr_obj;
        break;
      }
      curr_obj = curr_obj->queue.next;
    }

    // If we found an unpinned object, break the loop
    if (obj_to_evict != NULL) {
      break;
    }

    // Move to the next frequency node
    curr_freq_node = g_hash_table_lookup(params->freq_map, GSIZE_TO_POINTER(curr_freq_node->freq + 1));
  }

  // If no unpinned object is found in any frequency node, return -1 or any invalid object ID
  if (obj_to_evict == NULL) {
    return (obj_id_t)-1;
  }

  // Remove the object from its frequency node
  curr_freq_node->n_obj--;
  remove_obj_from_list(&curr_freq_node->first_obj, &curr_freq_node->last_obj, obj_to_evict);

  // Update cache metadata
  cache_evict_base(cache, obj_to_evict, true);

  // Update minimum frequency if necessary
  if (curr_freq_node->n_obj == 0) {
    DEBUG_ASSERT(curr_freq_node->first_obj == NULL);
    DEBUG_ASSERT(curr_freq_node->last_obj == NULL);

    if (curr_freq_node->freq == params->min_freq) {
      update_min_freq(params);
    }

    // Remove the empty frequency node from the frequency map
    g_hash_table_remove(params->freq_map, GSIZE_TO_POINTER(curr_freq_node->freq));
    free_freq_node(curr_freq_node);
  }

  // Return the ID of the evicted object
  return obj_to_evict->obj_id;
}

static inline void free_freq_node(void *list_node) { my_free(sizeof(freq_node_t), list_node); }
...
static void free_list_node(void *list_node) { my_free(sizeof(freq_node_t), list_node); }
