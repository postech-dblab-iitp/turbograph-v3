...
static obj_id_t ARC_evict_return_obj(cache_t *cache, const request_t *req);
...
static cache_obj_t *_ARC_to_evict_miss_on_all_queues(cache_t *cache, const request_t *req);
...
cache_t *ARC_init(const common_cache_params_t ccache_params, const char *cache_specific_params) {
  ...
  cache->evict_return_obj = ARC_evict_return_obj;  // Register the new function
...
static obj_id_t ARC_evict_return_obj(cache_t *cache, const request_t *req) {
  ARC_params_t *params = (ARC_params_t *)(cache->eviction_params);
  obj_id_t evicted_obj_id = -1;

  // Helper function to find the first unpinned object in a list
  cache_obj_t *find_unpinned_obj(cache_obj_t *head) {
    cache_obj_t *obj = head;
    while (obj != NULL) {
  ...
  // Function to evict from L1 or L2 data
  bool evict_from_data(bool from_L1) {
    cache_obj_t *obj_to_evict = find_unpinned_obj(from_L1 ? params->L1_data_tail : params->L2_data_tail);
    if (obj_to_evict) {
  ...
  if (params->vtime_last_req_in_ghost == cache->n_req &&
    ...
    if ((cond1 && (cond2 || cond3)) || cond4) {
      if (!evict_from_data(true) && !evict_from_data(false)) {
        return -1;  // All objects are pinned
      }
    } else {
      if (!evict_from_data(false) && !evict_from_data(true)) {
        return -1;  // All objects are pinned
      }
    }
  }
  // Case 2: Miss on all queues
  else {
