...
static void Hyperbolic_parse_params(cache_t *cache, const char *cache_specific_params);
static void Hyperbolic_free(cache_t *cache);
static bool Hyperbolic_get(cache_t *cache, const request_t *req);
static cache_obj_t *Hyperbolic_find(cache_t *cache, const request_t *req, const bool update_cache);
...
static obj_id_t Hyperbolic_evict_return_obj(cache_t *cache, const request_t *req);
...
cache_t *Hyperbolic_init(const common_cache_params_t ccache_params, const char *cache_specific_params) {
  ...
  cache->evict_return_obj = Hyperbolic_evict_return_obj;
...
static cache_obj_t *Hyperbolic_to_evict(cache_t *cache, const request_t *req) {
  ...
  for (int i = 0; i < params->n_sample; i++) {
    ...
    double age = (double)(cache->n_req - sampled_obj->hyperbolic.vtime_enter_cache);
 ...
 */
static void Hyperbolic_evict(cache_t *cache, __attribute__((unused)) const request_t *req) {
...
static cache_obj_t *Hyperbolic_to_evict_not_pinned(cache_t *cache, const request_t *req) {
  Hyperbolic_params_t *params = cache->eviction_params;
  cache_obj_t *best_candidate = NULL, *sampled_obj;
  double best_candidate_score = 1.0e16, sampled_obj_score;
  // It is possible to fail when all objects are pinned
  // Therefore, we need to avoid infinite loop
  int32_t try_count = 0;
  while (best_candidate == NULL) {
    ...
    for (int i = 0; i < params->n_sample; i++) {
      sampled_obj = hashtable_rand_obj(cache->hashtable);
      if (is_pinned(cache, sampled_obj->obj_id)) {
        continue;
      }
      double age = (double)(cache->n_req - sampled_obj->hyperbolic.vtime_enter_cache);
      sampled_obj_score = 1.0e8 * (double)sampled_obj->hyperbolic.freq / age;
...
static obj_id_t Hyperbolic_evict_return_obj(cache_t *cache, const request_t *req) {
  ...
  if (cache->to_evict_candidate_gen_vtime == cache->n_req) {
    ...
    obj_to_evict = Hyperbolic_to_evict_not_pinned(cache, req);
  }
  cache->to_evict_candidate_gen_vtime = -1;
...
static void Hyperbolic_remove_obj(cache_t *cache, cache_obj_t *obj) { cache_remove_obj_base(cache, obj, true); }
...
static void Hyperbolic_parse_params(cache_t *cache, const char *cache_specific_params) {
  ...
  while (params_str != NULL && params_str[0] != '\0') {
    ...
    if (strcasecmp(key, "n-sample") == 0) {
      ...
      ERROR("%s does not have parameter %s, support %s\n", cache->cache_name, key, Hyperbolic_current_params(params));
      exit(1);
    }
  }
