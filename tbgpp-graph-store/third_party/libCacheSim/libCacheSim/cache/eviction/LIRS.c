...
static cache_obj_t *LIRS_find(cache_t *cache, const request_t *req, const bool update_cache);
...
static obj_id_t LIRS_evict_return_obj(cache_t *cache, const request_t *req);
...
static cache_obj_t *hit_RD_HIRinS(cache_t *cache, cache_obj_t *cache_obj_s, cache_obj_t *cache_obj_q);
...
cache_t *LIRS_init(const common_cache_params_t ccache_params, const char *cache_specific_params) {
  ...
  cache->evict_return_obj = LIRS_evict_return_obj;
...
static cache_obj_t *LIRS_insert(cache_t *cache, const request_t *req) {
  ...
  if (obj_s != NULL && obj_s->LIRS.is_LIR == false && obj_s->LIRS.in_cache == false) {
...
// Modified evictHIR function
static obj_id_t evictHIR_(cache_t *cache) {
  LIRS_params_t *params = (LIRS_params_t *)(cache->eviction_params);
  cache_obj_t *obj_to_evict = ((LRU_params_t *)params->LRU_q->eviction_params)->q_tail;

  // Find the first unpinned HIR object
  while (obj_to_evict && (is_pinned(cache, obj_to_evict->obj_id) || obj_to_evict->LIRS.is_LIR)) {
    obj_to_evict = obj_to_evict->queue.prev;
  }

  if (obj_to_evict == NULL) {
    return -1;  // No unpinned HIR object found
  }

  obj_id_t evicted_id = obj_to_evict->obj_id;

  // Perform the eviction
  params->hirs_count -= obj_to_evict->obj_size;
  params->LRU_q->remove(params->LRU_q, evicted_id);
  ...
  cache_obj_t *obj_in_s = params->LRU_s->find(params->LRU_s, req_local, false);
  if (obj_in_s != NULL) {
    obj_in_s->LIRS.in_cache = false;
    params->LRU_nh->insert(params->LRU_nh, req_local);
...
// Modified evictLIR function
static obj_id_t evictLIR_(cache_t *cache) {
  LIRS_params_t *params = (LIRS_params_t *)(cache->eviction_params);
  cache_obj_t *obj_to_evict = ((LRU_params_t *)params->LRU_s->eviction_params)->q_tail;

  // Find the first unpinned LIR object
  while (obj_to_evict && (is_pinned(cache, obj_to_evict->obj_id) || !obj_to_evict->LIRS.is_LIR)) {
    obj_to_evict = obj_to_evict->queue.prev;
  }

  if (obj_to_evict == NULL) {
    return -1;  // No unpinned LIR object found
  }
  ...
  // Perform the eviction
  params->lirs_count -= obj_to_evict->obj_size;
  params->LRU_s->remove(params->LRU_s, evicted_id);

  cache->occupied_byte -= (obj_to_evict->obj_size + cache->obj_md_size);
  cache->n_obj -= 1;

  if (obj_to_evict->obj_size <= params->hirs_limit) {
    // Move to HIR in Q
    static __thread request_t *req_local = NULL;
    if (req_local == NULL) {
      req_local = new_request();
    ...
    params->LRU_q->insert(params->LRU_q, req_local);
    cache_obj_t *new_hir = params->LRU_q->find(params->LRU_q, req_local, false);
    new_hir->LIRS.is_LIR = false;
    new_hir->LIRS.in_cache = true;
    params->hirs_count += new_hir->obj_size;
    cache->occupied_byte += (new_hir->obj_size + cache->obj_md_size);
    cache->n_obj += 1;
    evicted_id = -1;
  }
...
static obj_id_t LIRS_evict_return_obj(cache_t *cache, const request_t *req) {
  ...
  // First, try to evict an HIR object
  evicted_obj_id = evictHIR_(cache);
  if (evicted_obj_id != -1) {
    return evicted_obj_id;
  }

  // If no HIR object could be evicted, try to evict an LIR object
  evicted_obj_id = evictLIR_(cache);
  if (evicted_obj_id != -1) {
    return evicted_obj_id;
  }

  // If we reach here, all objects are pinned
  return -1;
}
...
static void LIRS_print_cache(cache_t *cache) {
  ...
  printf("S Stack:  %lu:%lu %lu:%lu \n", (unsigned long)params->lirs_limit, (unsigned long)params->lirs_count,
         (unsigned long)params->hirs_limit, (unsigned long)params->hirs_count);
  cache_obj_t *obj = ((LRU_params_t *)params->LRU_s->eviction_params)->q_head;
  while (obj) {
    printf("%ld(%u, %s, %s)->", (long)obj->obj_id, obj->obj_size, obj->LIRS.in_cache ? "R" : "N",
           obj->LIRS.is_LIR ? "L" : "H");
    obj = obj->queue.next;
  }
  printf("\n");
  ...
  while (obj_q) {
    printf("%ld(%u, %s, %s)->", (long)obj_q->obj_id, obj_q->obj_size, obj_q->LIRS.in_cache ? "R" : "N",
  ...
  printf("NH Stack: \n");
  cache_obj_t *obj_nh = ((LRU_params_t *)params->LRU_nh->eviction_params)->q_head;
  while (obj_nh) {
    printf("%ld(%u, %s, %s)->", (long)obj_nh->obj_id, obj_nh->obj_size, obj_nh->LIRS.in_cache ? "R" : "N",
...
static void LIRS_print_cache_compared_to_cacheus(cache_t *cache) {
  ...
  while (obj) {
    printf("(o=%ld, is_LIR=%s, in_cache=%s)\n", (long)obj->obj_id, obj->LIRS.is_LIR ? "True" : "False",
  ...
  while (obj_q) {
    printf("(o=%ld, is_LIR=%s, in_cache=%s)\n", (long)obj_q->obj_id, obj_q->LIRS.is_LIR ? "True" : "False",
           obj_q->LIRS.in_cache ? "True" : "False");
    obj_q = obj_q->queue.prev;
  }

  printf("NH:\n");
  cache_obj_t *obj_nh = ((LRU_params_t *)params->LRU_nh->eviction_params)->q_tail;
  while (obj_nh) {
    printf("(o=%ld, is_LIR=%s, in_cache=%s)\n", (long)obj_nh->obj_id, obj_nh->LIRS.is_LIR ? "True" : "False",
