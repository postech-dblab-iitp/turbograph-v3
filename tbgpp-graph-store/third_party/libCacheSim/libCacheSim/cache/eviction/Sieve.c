...
static obj_id_t Sieve_evict_return_obj(cache_t *cache, const request_t *req);
...
cache_t *Sieve_init(const common_cache_params_t ccache_params,
  ...
  cache->evict_return_obj = Sieve_evict_return_obj;
...
static obj_id_t Sieve_evict_return_obj(cache_t *cache, const request_t *req) {
  ...
  int to_evict_freq = 0;
  cache_obj_t *obj;
  cache_obj_t *start_obj;
  int i;

  while (evicted_obj_id == -1) {
    /* if we have run one full around or first eviction */
    obj = params->pointer == NULL ? params->q_tail : params->pointer;
    start_obj = obj;

    do {
      if (obj->sieve.freq == to_evict_freq && !is_pinned(cache, obj->obj_id)) {
        /* Found an unpinned object with the current eviction frequency */
        evicted_obj_id = obj->obj_id;
    ...
    /* If we've completed a full cycle without finding an object to evict,
       decrement frequencies and increase the eviction frequency threshold */
    obj = params->q_tail;
    while (obj != NULL) {
      if (obj->sieve.freq > 0) {
        obj->sieve.freq -= 1;
    ...
    /* If we've checked all possible frequencies, break to avoid an infinite loop */
    if (to_evict_freq > cache->get_n_obj(cache)) {
      break;
    }
  }

  /* If we reach here, all objects are pinned or there's an unexpected state */
  return evicted_obj_id;  /* Will be -1 */
}
