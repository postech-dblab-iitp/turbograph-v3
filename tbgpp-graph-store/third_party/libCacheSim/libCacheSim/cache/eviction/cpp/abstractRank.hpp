...
class abstractRank {
 ...
 public:
  ...
  inline pq_node_type find_erase_unpinned_victim(cache_t *cache) {
    for (auto it = pq.begin(); it != pq.end(); ++it) {
      if (!is_pinned(cache, it->obj->obj_id)) {
        pq_node_type p_copy(*it);
        itr_map.erase(it->obj);
        pq.erase(it);
        return std::move(p_copy);
      }
    }
    // Return an invalid node if all objects are pinned
    return pq_node_type(nullptr, 0, -1);
  }
