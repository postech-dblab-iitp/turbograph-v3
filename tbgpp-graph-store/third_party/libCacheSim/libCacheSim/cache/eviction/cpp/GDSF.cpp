...
static obj_id_t GDSF_evict_return_obj(cache_t *cache, const request_t *req);
...
cache_t *GDSF_init(const common_cache_params_t ccache_params, const char *cache_specific_params) {
  ...
  cache->evict_return_obj = GDSF_evict_return_obj;
  cache->to_evict = NULL;
 ...
 */
static bool GDSF_get(cache_t *cache, const request_t *req) { return cache_get_base(cache, req); }
...
static cache_obj_t *GDSF_find(cache_t *cache, const request_t *req, const bool update_cache) {
  ...
  if (obj != nullptr && update_cache) {
    ...
    gdsf->pq.erase(itr);
    // The weird-lookign " (obj->obj_size) / obj->obj_size;" is to target the byte hit ratio.
    double pri = gdsf->pri_last_evict + (double)(obj->lfu.freq) * (obj->obj_size) / obj->obj_size;
...
static cache_obj_t *GDSF_insert(cache_t *cache, const request_t *req) {
  ...
  double pri = gdsf->pri_last_evict + (obj->obj_size) / obj->obj_size;
...
static obj_id_t GDSF_evict_return_obj(cache_t *cache, const request_t *req) {
  ...
  eviction::pq_node_type victim = gdsf->find_erase_unpinned_victim(cache);
  ...
  // Update the last evicted priority
  gdsf->pri_last_evict = victim.priority;

  // Get the object ID before removing the object from the cache
  obj_id_t evicted_obj_id = victim.obj->obj_id;

  // Remove the object from the priority queue and itr_map
  // gdsf->pq.erase(gdsf->itr_map[victim.obj]);
  // gdsf->itr_map.erase(victim.obj);
