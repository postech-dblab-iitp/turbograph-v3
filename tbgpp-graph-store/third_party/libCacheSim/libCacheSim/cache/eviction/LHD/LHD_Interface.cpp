...
static cache_obj_t *LHD_find(cache_t *cache, const request_t *req, const bool update_cache);
...
static obj_id_t LHD_evict_return_obj(cache_t *cache, const request_t *req);
...
cache_t *LHD_init(const common_cache_params_t ccache_params, const char *cache_specific_params) {
  ...
  cache->evict_return_obj = LHD_evict_return_obj;
 ...
 */
static bool LHD_get(cache_t *cache, const request_t *req) { return cache_get_base(cache, req); }
...
static cache_obj_t *LHD_find(cache_t *cache, const request_t *req, const bool update_cache) {
  ...
  if (update_cache) {
    // We don't care about the object size here
...
static obj_id_t LHD_evict_return_obj(cache_t *cache, const request_t *req) {
  ...
  repl::candidate_t victim;
  uint32_t candidates = lhd->ASSOCIATIVITY;
  std::vector<repl::candidate_t> ranked_candidates;
  ...
  for (uint32_t i = 0; i < candidates; i++) {
    ...
    auto idx = next_rand() % lhd->tags.size();
    auto &tag = lhd->tags[idx];
    ranked_candidates.push_back(tag.id);
  }

  // Sort candidates based on hit density (ascending order)
  std::sort(ranked_candidates.begin(), ranked_candidates.end(),
            [&](const repl::candidate_t &a, const repl::candidate_t &b) {
              return lhd->getHitDensity(lhd->tags[lhd->indices[a]]) < lhd->getHitDensity(lhd->tags[lhd->indices[b]]);
            });

  // Find the first unpinned candidate
  bool found_victim = false;
  for (const auto &candidate : ranked_candidates) {
    if (!is_pinned(cache, candidate.id)) {
      victim = candidate;
      found_victim = true;
      break;
    }
  ...
  // If all candidates are pinned or no candidates available, return invalid object id
  if (!found_victim) {
    return -1;
  }
  ...
  obj_id_t evicted_obj_id = victim.id;
  int64_t evicted_size = victimItr->second + cache->obj_md_size;

  // Ensure we're not underflowing
  if (cache->occupied_byte >= evicted_size) {
    cache->occupied_byte -= evicted_size;
  } else {
    std::cerr << "Warning: Attempting to evict more bytes than occupied. Resetting to 0." << std::endl;
    cache->occupied_byte = 0;
  }

  if (cache->n_obj > 0) {
    cache->n_obj -= 1;
  } else {
    std::cerr << "Warning: Attempting to reduce object count below 0. Keeping at 0." << std::endl;
  }
...
static bool LHD_remove(cache_t *cache, const obj_id_t obj_id) {
  ...
  hashtable_delete_obj_id(cache->hashtable, obj_id);

  return true;
}

static int64_t LHD_get_occupied_byte(const cache_t *cache) { return cache->occupied_byte; }
