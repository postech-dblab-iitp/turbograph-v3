...
namespace lrb {
    ...
    static const uint32_t batch_size = 1000; // tjyoon, this value is reduced since I will use a workload with smaller size (~=100000)
...
//                n_edc_feature = stoull(it.second);
    ...
    uint64_t evict_return_obj(cache_t *cache);

    uint64_t evict_with_candidate_return_obj(pair<uint64_t, uint32_t> &epair);
    ...
    pair<uint64_t, uint32_t> rank_with_pinning(cache_t *cache);
