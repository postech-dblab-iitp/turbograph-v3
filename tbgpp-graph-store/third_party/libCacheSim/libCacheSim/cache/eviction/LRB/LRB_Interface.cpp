...
static cache_obj_t *LRB_find(cache_t *cache, const request_t *req, const bool update_cache);
...
static obj_id_t LRB_evict_return_obj(cache_t *cache, const request_t *req);
...
cache_t *LRB_init(const common_cache_params_t ccache_params, const char *cache_specific_params) {
  ...
  cache->evict_return_obj = LRB_evict_return_obj;
 ...
 */
static bool LRB_get(cache_t *cache, const request_t *req) { return cache_get_base(cache, req); }
...
static cache_obj_t *LRB_insert(cache_t *cache, const request_t *req) {
  ...
  if (hashtable_find_obj_id(cache->hashtable, req->obj_id) != NULL) {
    cache_obj = hashtable_find_obj_id(cache->hashtable, req->obj_id);
  } else {
    cache_obj = hashtable_insert(cache->hashtable, req);
  }
  cache_obj->pin_count = 0;
...
static obj_id_t LRB_evict_return_obj(cache_t *cache, const request_t *req) {
  ...
  if (cache->to_evict_candidate_gen_vtime == cache->n_req) {
    ...
    lrb->evict_with_candidate(params->to_evict_pair);
    cache->to_evict_candidate_gen_vtime = -1;
    return params->to_evict_pair.first;
  } else {
    evicted_obj_id = lrb->evict_return_obj(cache);
  }
  // printf("evicting %ld\n", evicted_obj_id);
