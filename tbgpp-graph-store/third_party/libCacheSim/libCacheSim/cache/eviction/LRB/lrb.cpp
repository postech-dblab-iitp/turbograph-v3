...
void LRBCache::train() {
  ...
  LGBM_DatasetCreateFromCSR(static_cast<void *>(training_data->indptr.data()), C_API_DTYPE_INT32,
                            training_data->indices.data(), static_cast<void *>(training_data->data.data()),
                            C_API_DTYPE_FLOAT64, training_data->indptr.size(), training_data->data.size(),
                            n_feature,  // remove future t
                            map_to_string(training_params).c_str(), nullptr, &trainData);

  LGBM_DatasetSetField(trainData, "label", static_cast<void *>(training_data->labels.data()),
  ...
  LGBM_BoosterPredictForCSR(booster, static_cast<void *>(training_data->indptr.data()), C_API_DTYPE_INT32,
                            training_data->indices.data(), static_cast<void *>(training_data->data.data()),
                            C_API_DTYPE_FLOAT64, training_data->indptr.size(), training_data->data.size(),
                            n_feature,  // remove future t
                            C_API_PREDICT_NORMAL, 0, atoi(training_params["num_iterations"].c_str()),
                            map_to_string(training_params).c_str(), &len, result.data());
...
void LRBCache::update_stat_periodic() {
  ...
  if (0 == training_data_distribution[0] && 0 == training_data_distribution[1]) {
    ...
    positive_example_ratio = static_cast<float>(training_data_distribution[1]) /
                             (training_data_distribution[0] + training_data_distribution[1]);
  }
...
bool LRBCache::lookup(const SimpleRequest &req) {
  ...
  if (it != key_map.end()) {
    ...
    if (list_idx && negative_candidate_queue->find(forget_timestamp) == negative_candidate_queue->end()) {
      printf("Debug print\n");
      printf("current_seq: %lu\n", current_seq);
      printf("last_timestamp: %lu\n", last_timestamp);
      printf("forget_timestamp: %lu\n", forget_timestamp);
      for (auto &item : *negative_candidate_queue) {
        printf("%lu %lu\n", item.first, item.second);
      }
    }
    assert((!list_idx) || (negative_candidate_queue->find(forget_timestamp) != negative_candidate_queue->end()));
    // re-request
    if (!meta._sample_times.empty()) {
      ...
      for (auto &sample_time : meta._sample_times) {
        ...
        training_data->emplace_back(meta, sample_time, future_distance, meta._key, max_hash_edc_idx, edc_windows,
                                    hash_edc);
    ...
    if (list_idx) {
      ...
      assert(negative_candidate_queue->find(current_seq % memory_window) != negative_candidate_queue->end());
    } else {
...
pair<uint64_t, uint32_t> LRBCache::rank() {
  ...
  while (idx_row != sample_rate) {
    ...
    for (uint8_t k = 0; k < n_edc_feature; ++k) {
      ...
      uint32_t _distance_idx = min(uint32_t(current_seq - meta._past_timestamp) / edc_windows[k], max_hash_edc_idx);
      if (meta._extra)
  ...
  LGBM_BoosterPredictForCSR(booster, static_cast<void *>(indptr), C_API_DTYPE_INT32, indices, static_cast<void *>(data),
                            C_API_DTYPE_FLOAT64, idx_row + 1, idx_feature,
                            n_feature,  // remove future t
                            C_API_PREDICT_NORMAL, 0, atoi(training_params["num_iterations"].c_str()),
  ...
  if (!(current_seq % 10000))
    inference_time =
        0.95 * inference_time +
        0.05 * chrono::duration_cast<chrono::microseconds>(chrono::system_clock::now() - timeBegin).count();
  ...
  if (objective == object_miss_ratio) {
    for (uint32_t i = 0; i < sample_rate; ++i) scores[i] *= sizes[i];
  }
  ...
  sort(index.begin(), index.end(), [&](const int &a, const int &b) { return (scores[a] > scores[b]); });

  return {keys[index[0]], poses[index[0]]};
}

pair<uint64_t, uint32_t> LRBCache::rank_with_pinning(cache_t *cache) {
  {
    // if not trained yet, or in_cache_lru past memory window, use LRU
    auto &candidate_key = in_cache_lru_queue.dq.back();
    auto it = key_map.find(candidate_key);
    ...
    if ((!booster) || (memory_window <= current_seq - meta._past_timestamp)) {
      // this use LRU force eviction, consider sampled a beyond boundary object
      for(auto it = in_cache_lru_queue.dq.rbegin(); it != in_cache_lru_queue.dq.rend(); ++it) {
        auto key = *it;
        auto it_ = key_map.find(key);
        auto pos_ = it_->second.list_pos;
        auto &meta_ = in_cache_metas[pos_];
        if (!is_pinned(cache, meta_._key)) {
          if (booster) {
            ++obj_distribution[1];
          }
          return {meta_._key, pos_};
        }
      }
      return {-1, -1};
    }
  ...
  sort(index.begin(), index.end(), [&](const int &a, const int &b) {
    if (is_pinned(cache, keys[a]) && !is_pinned(cache, keys[b]))
      return false;
    else if (!is_pinned(cache, keys[a]) && is_pinned(cache, keys[b]))
      return true;
    else
      return (scores[a] > scores[b]);
  });
  if (is_pinned(cache, keys[index[0]])) {
    return {-1, -1};
  }
  return {keys[index[0]], poses[index[0]]};
}
...
uint64_t LRBCache::evict_return_obj(cache_t *cache) {
  auto epair = rank_with_pinning(cache);
  if (epair.first == -1) {
    return -1;
  }
  evict_with_candidate(epair);
...
void LRBCache::evict_with_candidate(pair<uint64_t, uint32_t> &epair) {
  ...
  if (memory_window <= current_seq - meta._past_timestamp) {
    ...
    // printf("Eviction key: %lu\n", in_cache_metas[old_pos]._key);
    uint32_t activate_tail_idx = in_cache_metas.size() - 1;
    if (old_pos != activate_tail_idx) {
      // move tail
      // printf("Activate tail key: %lu, old_pos: %lu\n, overwriting key %lu, received key: %lu\n", in_cache_metas[activate_tail_idx]._key, old_pos, in_cache_metas[old_pos]._key, key);
      if(in_cache_metas[old_pos]._key != key) {
...
uint64_t LRBCache::evict_with_candidate_return_obj(pair<uint64_t, uint32_t> &epair) {
  is_sampling = true;
