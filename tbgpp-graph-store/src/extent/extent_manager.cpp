...
//Below code is copied from _AppendChunkToExtentWithCompression().
...
ExtentID ExtentManager::GenerateExtentFromChunkToSend(DataChunk& input, int32_t file_seq_number, ProcessID dest) { //context, prop_schema_cat_entry
    //This function is called by master node of graph partitioning. 
    //Master node read given DataChunk, and allocate SimpleContainer with appropriate size using RequestRespond's method.
    //Then, master node will push the SimpleContainer to the queue of the destination node.
    ...
    for (auto &l_type : input.GetTypes()) {
        ...
        string chunkdefinition_name = "cdf_" + std::to_string(cdf_id);
        CreateChunkDefinitionInfo chunkdefinition_info("main", chunkdefinition_name, l_type);
        ChunkDefinitionCatalogEntry* chunkdefinition_cat = (ChunkDefinitionCatalogEntry*) cat_instance.CreateChunkDefinition(*GraphPartitioner::client, &chunkdefinition_info);
        extent_cat_entry->AddChunkDefinitionID(cdf_id);
        chunkdefinition_cat->SetNumEntriesInColumn(input.size());
