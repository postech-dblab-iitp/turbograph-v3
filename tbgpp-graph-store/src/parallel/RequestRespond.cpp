...
//     D_ASSERT (my_core_id_ != -1);
//     D_ASSERT (my_core_id_ >= 0 && my_core_id_ < MAX_NUM_PER_THREAD_DATASTRUCTURE);
...
SimpleContainer RequestRespond::GetTempDataBuffer(int64_t req_size) {
	...
	D_ASSERT (cont.capacity >= req_size);
	cont.size_used = 0;
...
void RequestRespond::ReturnTempDataBuffer(SimpleContainer cont) {
	...
	if (cont.capacity > max_dynamic_buffer_size) {
		...
		D_ASSERT (cont.capacity == large_data_size);
		large_data_buffer_queue_.enqueue(cont);
	} else {
		D_ASSERT (cont.capacity == small_data_size);
...
void RequestRespond::ReturnTempNodeBitMap(bool sender_or_receiver, TwoLevelBitMap<node_t>* tmp_bitmap) {
    ...
    D_ASSERT (tmp_bitmap != nullptr);
	tmp_bitmap->ClearAll();
    int queue_idx = (sender_or_receiver == true) ? 0 : 1;
	temp_bitmap_queue_[queue_idx].enqueue(tmp_bitmap);
}

void RequestRespond::PartitionedExtentRead(int32_t size, int from, int send_count) {
	D_ASSERT(from == 0);
	GraphPartitioner::ReceiveAndStoreExtent(size, send_count);
}

void RequestRespond::GenerateDirectory(duckdb::PartitionID pid, int from) {
	D_ASSERT(from == 0);
	std::string dir_path_str = DiskAioParameters::WORKSPACE + "/part_" + std::to_string(pid);
	printf("Segment/ Trying GenerateDirectory %s\n", dir_path_str.c_str());
	duckdb::MkDir(dir_path_str, false);
}
...
//     D_ASSERT(is_full_list == (UserArguments::ITERATOR_MODE == IteratorMode_t::FULL_LIST));
//     D_ASSERT(bitmap_first_vid == PartitionStatistics::my_first_node_id());
...
//     D_ASSERT(alloc_size_for_CurPid < 4 * 1024 * 1024L);
...
// 			D_ASSERT (adjlist_data_sz != 0);	// Assuming undirected graph.
...
//             D_ASSERT(mat_adj.GetNumAdjacencyLists() == slot_data_size_cache / sizeof(Slot32));
//             D_ASSERT(mat_adj.GetNumEdges() == adjlist_data_size_cache / sizeof(node_t));
//             D_ASSERT(mat_adj.GetNumAdjacencyLists() != 0 && mat_adj.GetNumEdges() != 0);
//             D_ASSERT(vid_range.contains(mat_adj.GetSrcVidRange()));
...
//         }
// 		D_ASSERT (!((adjlist_data_sz + sizeof(Slot32) + accm_page_metadata_size) > MaxBytesToSend));
...
// 		total_bytes += size_to_request;
// 	}
// 	D_ASSERT (total_bytes <= MaxBytesToSend);
...
//             D_ASSERT(version_range.GetEnd() >= 0);
//             //CurPid[dbtype].resize(version_range.GetEnd() + 1);
//             D_ASSERT(CurPid[dbtype].size() == version_range.GetEnd() + 1);
...
//                 D_ASSERT(CurPid[dbtype][version_id].size() == PartitionStatistics::my_total_num_subchunks());
...
//                     D_ASSERT(!TurboDB::GetTurboDB(e_type)->is_version_exist((DynamicDBType)dbtype, version_id));
//                     continue;
//                 }
...
//     D_ASSERT(UserArguments::USE_DEGREE_ORDER_REPRESENTATION);
//     D_ASSERT(vertices.size() == slot_data_size_cache / sizeof(Slot32));
//     D_ASSERT (VidToAdjacencyListMappingTable::VidToAdjTable.IsValid(0) || VidToAdjacencyListMappingTable::VidToAdjTable.BeingReferenced(0));
...
//         D_ASSERT(VidToAdjacencyListMappingTable::VidToAdjTable.Has(vid, win_lv));
//         D_ASSERT(vid_range.contains(vid));
...
//         D_ASSERT (vid == adj_ptr->GetSlotVid(ptr.slot_id));
// #ifndef PERFORMANCE
//         if (UserArguments::USE_DELTA_NWSM) {
//             D_ASSERT (TurboDB::OutDegree(vid) - TurboDB::OutInsertDegree(vid) + TurboDB::OutDeleteDegree(vid) + TurboDB::OutInsertDegree(vid) + TurboDB::OutDeleteDegree(vid) == sz);
//         } else {
//             D_ASSERT (TurboDB::OutDegree(vid) == sz);
//         }
// #endif
//         D_ASSERT (current_data_offset * sizeof(node_t) + sz * sizeof(node_t) <= adjlist_data_size_cache);
...
//         D_ASSERT (copy_to >= tmp_container && copy_to < tmp_container + size_to_request_cache);
//         D_ASSERT (copy_from >= adj_ptr->Data() && copy_from < adj_ptr->Data() + adj_ptr->Size());
...
void RequestRespond::ReceiveRequest() {
	...
	while(true) {
        ...
        tim.start_timer(1);
		D_ASSERT (stat.MPI_SOURCE >= 0 && stat.MPI_SOURCE < PartitionStatistics::num_machines());
        ...
        D_ASSERT(0 < recv_bytes <= max_request_size);
        ...
        switch(((Request *)tmp_buffer)->rt) {
			...
			case GenerateDirectoryMessage: {
				GenerateDirectoryRequest* tmp_r = (GenerateDirectoryRequest*) tmp_buffer;
				respond = rr_.async_resp_pool.enqueue(RequestRespond::GenerateDirectory, tmp_r->pid, tmp_r->from);
				is_working = true;
			...
			case PartitionedExtentReadMessage: {
                ...
                respond = rr_.async_resp_pool.enqueue(RequestRespond::PartitionedExtentRead, tmp_r->size, tmp_r->from, tmp_r->send_count);
                is_working = true;
                ReturnTempReqReceiveBuffer(tmp_buffer);
                break;
			}
...
void RequestRespond::SendDataInBuffer(tbb::concurrent_queue<SimpleContainer>* req_data_queue, turbo_tcp* vector_client_sockets, int partition_id, int tag, int chunk_idx, int lv, int64_t eom_cnt, bool delete_req_buffer_queue) {
	D_ASSERT (chunk_idx >= 0 && chunk_idx < UserArguments::VECTOR_PARTITIONS);
	D_ASSERT (req_data_queue != NULL);
    D_ASSERT (!UserArguments::USE_PULL);
...
Start:
	while (true) {
		...
		if (cont_combined.size_available() == 0) {
			...
			D_ASSERT (bytes_sent == cont_combined.size_used);
		...
		if (size_to_combine != cont.size_used) {
			D_ASSERT(size_to_combine < cont.size_used);
			D_ASSERT(cont_combined.size_used == 0);
...
template void RequestRespond::SendRequest<GenerateDirectoryRequest>(int partition_id, GenerateDirectoryRequest req_buffer);
...
void RequestRespond::SendRequest(Request* req, int num_reqs, char* data_buffer[], int64_t data_size[], int64_t num_data_buffers) {
    ...
    for (int64_t idx = 0; idx < num_data_buffers; idx++) {
        ...
        D_ASSERT(bytes_sent == data_size[idx]);
    }
