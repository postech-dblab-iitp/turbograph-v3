...
// vector<std::pair<string, string>> GraphPartitioner::json_files;
// vector<JsonFileType> GraphPartitioner::json_file_types;
// vector<vector<std::pair<string, string>>> GraphPartitioner::json_file_vertices;
// vector<vector<std::pair<string, string>>> GraphPartitioner::json_file_edges;
vector<std::pair<string, string>> GraphPartitioner::vertex_files;
vector<std::pair<string, string>> GraphPartitioner::edge_files;
vector<std::pair<string, string>> GraphPartitioner::edge_files_backward;
string GraphPartitioner::output_dir;
...
std::thread* GraphPartitioner::rr_receiver_;
...
vector<std::pair<string, unordered_map<LidPair, idx_t, boost::hash<LidPair>>>> GraphPartitioner::lid_to_pid_map;
std::vector<atom> GraphPartitioner::lid_to_pid_map_locks;
vector<std::pair<string, unordered_map<LidPair, idx_t, boost::hash<LidPair>>>> GraphPartitioner::lid_pair_to_epid_map; // For Backward AdjList
...
void GraphPartitioner::ReceiveAndStoreExtent(int size) {
    ...
    int recv_size = client_sockets.recv_from_server((char*) cont.data, 0, src_process_id, true); //This also sends the directory that extent should be located.
    ...
    while(1) {
        if(per_generator_datachunk_queue[my_generator_id].try_pop(datachunk_with_file_seq_number)) { 
            ...
            if(load_edge && file_meta_infos[file_seq_number].file_type == GraphFileType::Vertex) { //TODO: check file type and add only if vertex file. 
                ...
                auto key_column_idxs = file_meta_infos[file_seq_number].key_column_idxs;
				...
				if (key_column_idxs.size() == 1) {
					...
					for (idx_t seqno = 0; seqno < datachunk_with_file_seq_number.second->size(); seqno++) {
						...
						local_lid_to_pid_map_instance.emplace(lid_key, pid_base + seqno);
					}
				} else if (key_column_idxs.size() == 2) {
					idx_t *key_column_1 = (idx_t *)datachunk_with_file_seq_number.second->data[file_meta_infos[file_seq_number].key_column_idxs[0]].GetData();
					idx_t *key_column_2 = (idx_t *)datachunk_with_file_seq_number.second->data[file_meta_infos[file_seq_number].key_column_idxs[1]].GetData();
                ...
                std::unordered_map <LidPair, duckdb::idx_t, boost::hash<LidPair>> *global_lid_to_pid_map_instance = &lid_to_pid_map[file_seq_number].second; 
...
void GraphPartitioner::ReadVertexFileAndCreateDataChunk(DistributionPolicy policy, std::vector<std::string> hash_columns, std::pair<std::string, std::string> fileinfo)
{
    ...
    string &vertex_labelset = fileinfo.first;
    string &vertex_file_path = fileinfo.second;
    ...
    file_meta_info.part_cat = partition_cat;
...
void GraphPartitioner::CreateVertexCatalogInfos(Catalog &cat_instance, std::shared_ptr<ClientContext> client, GraphCatalogEntry *graph_cat,
...
{   
    if(role == Role::MASTER) {
        //Send Exit message: notice segments that partitioning process is finished.
        //Assume that MPI messages are arrived in send order.
        for(ProcessID dest_process_id = 1; dest_process_id < process_count; dest_process_id++) {
            ExitRequest request;
