  ...
  public:
    ...
    */
    void Init(Idx_t num_entries) {
        ...
        D_ASSERT ( (sizeof(bitmap_entry_t) * ((num_entries + SizeInBits(bitmap_entry_t) - 1) / SizeInBits(bitmap_entry_t))) >= (num_entries + SizeInBits(bitmap_entry_t) - 1) / SizeInBits(bitmap_entry_t));
...
#pragma omp parallel for
    ...
    */
	void CopyFrom(BitMap<Idx_t>& from) {
		D_ASSERT (array_size == from.array_size);
        D_ASSERT(entries_ != NULL && from.entries_ != NULL);
		#pragma omp parallel for
    ...
    */
	int64_t GetTotalInRange(Idx_t from, Idx_t to) const {
		D_ASSERT(from >= 0 && from / SizeInBits(bitmap_entry_t) < array_size);
		D_ASSERT(to >= 0 && to / SizeInBits(bitmap_entry_t) < array_size);
    ...
    */
	int64_t GetTotalInRangeSingleThread(Idx_t from, Idx_t to) const {
		D_ASSERT(0 <= from && from / SizeInBits(bitmap_entry_t) < array_size);
		D_ASSERT(0 <= to && to / SizeInBits(bitmap_entry_t) < array_size);
		int64_t total_cnts = 0;
		for (Idx_t i = from; i <= to; i++) {
			total_cnts += Get(i);
		}
    ...
    */
    ...
    inline bool operator[](int64_t idx) {
        D_ASSERT(0 <= idx);
        D_ASSERT((idx) / SizeInBits(bitmap_entry_t) < array_size);
	...
	inline void Set(Idx_t idx) {
		D_ASSERT(idx >= 0 &&(idx) / SizeInBits(bitmap_entry_t) < array_size);
		Idx_t bit_pos = idx / SizeInBits(bitmap_entry_t);
		Idx_t bit_loc = idx % SizeInBits(bitmap_entry_t);
	...
	inline void ClearUnsafe(Idx_t idx) {
		D_ASSERT(0 <= idx && (idx) / SizeInBits(bitmap_entry_t) < array_size);
		Idx_t bit_pos = idx / SizeInBits(bitmap_entry_t);
		Idx_t bit_loc = idx % SizeInBits(bitmap_entry_t);
    ...
    */
	...
	inline bitmap_entry_t GetSetBits(Idx_t idx_at, Idx_t from_bit, Idx_t to_bit) const {
        D_ASSERT(from_bit >= 0 && from_bit < SizeInBits(bitmap_entry_t));
        D_ASSERT(to_bit >= 0 && to_bit < SizeInBits(bitmap_entry_t));
        D_ASSERT(from_bit <= to_bit);
...
#pragma omp for schedule (dynamic, 16*64)
    ...
    void SwapBitMap(BitMap<Idx_t>& another) {
        D_ASSERT (num_entries_ == another.num_entries_);
