...
void InitializeDiskAio() {
	...
	DiskAioParameters::WORKSPACE = GraphPartitioner::output_dir;
...
void ReadFwdEdgeCSVFileAndCreateEdgeExtents(Catalog &cat_instance, ExtentManager &ext_mng, std::shared_ptr<ClientContext> client, GraphCatalogEntry *&graph_cat,
	...
	for (auto &edge_file: GraphPartitioner::edge_files) {
		...
		unordered_map<LidPair, idx_t, boost::hash<LidPair>> *lid_pair_to_epid_map_instance;
		if (GraphPartitioner::load_backward_edge) {
		...
		while (!reader.ReadCSVFile(key_names, types, data)) {
			...
			if (src_column_idx.size() == 1) {
				while (src_seqno < max_seqno) {
					...
					if (src_key == prev_id) {
						...
						end_idx = src_seqno;
						FillAdjListBuffer(GraphPartitioner::load_backward_edge, begin_idx, end_idx, src_seqno, prev_src_pid, vertex_seqno,
			...
			end_idx = src_seqno;
			FillAdjListBuffer(GraphPartitioner::load_backward_edge, begin_idx, end_idx, src_seqno, cur_src_pid, vertex_seqno,
							  dst_column_idx, dst_key_columns, dst_lid_to_pid_map_instance,
							  lid_pair_to_epid_map_instance, adj_list_buffers, epid_base);
...
void ReadBwdEdgeCSVFileAndCreateEdgeExtents(Catalog &cat_instance, ExtentManager &ext_mng, std::shared_ptr<ClientContext> client, GraphCatalogEntry *&graph_cat,
	...
	for (auto &edge_file: GraphPartitioner::edge_files_backward) {
		auto edge_file_start = std::chrono::high_resolution_clock::now();
		...
		while (!reader.ReadCSVFile(key_names, types, data)) {
			...
			if (src_column_idx.size() == 1) {
				while (src_seqno < max_seqno) {
					...
					if (src_key == prev_id) {
						...
						end_idx = src_seqno;
						FillBwdAdjListBuffer(GraphPartitioner::load_backward_edge, begin_idx, end_idx, src_seqno, prev_src_pid, vertex_seqno,
										  dst_column_idx, dst_key_columns, dst_lid_to_pid_map_instance,
			...
			// Process remaining dst vertices
			FillBwdAdjListBuffer(GraphPartitioner::load_backward_edge, begin_idx, end_idx, src_seqno, cur_src_pid, vertex_seqno,
										  dst_column_idx, dst_key_columns, dst_lid_to_pid_map_instance,
										  lid_pair_to_epid_map_instance, adj_list_buffers);
		}
    ...
    void ParseCmdOption() const {
    	...
    	for (itr = this->tokens.begin(); itr != this->tokens.end(); itr++) {
    		...
    		if (std::strncmp(current_str.c_str(), "--nodes:", 8) == 0) {
				...
				GraphPartitioner::vertex_files.push_back(pair_to_insert);
			} else if (std::strncmp(current_str.c_str(), "--relationships:", 16) == 0) {
				std::pair<std::string, std::string> pair_to_insert;
				pair_to_insert.first = std::string(*itr).substr(16);
				itr++;
				pair_to_insert.second = *itr;
				GraphPartitioner::edge_files.push_back(pair_to_insert);
				GraphPartitioner::load_edge = true;
				...
				itr++;
				pair_to_insert.second = *itr;
				GraphPartitioner::edge_files_backward.push_back(pair_to_insert);
				GraphPartitioner::load_backward_edge = true;
			} 
            /* Currently do not care json. 
...
int main(int argc, char** argv) {
    ...
    GraphCatalogEntry *graph_cat = NULL;
    ...
    if(GraphPartitioner::role == Role::MASTER) { //Only Master process file. Segment's operations will be covered by RequestRespond's respond functions.
		...
		GraphPartitioner::file_reading_finished.store(true);
